...
| ~~`VacationRequest.status`~~ | **해소(V96)** | enum(PENDING/APPROVED/REJECTED) + CHECK 로 바꿨다. API 는 `status`(enum) + `statusName`(한글) 관례를 따른다 |
| ~~`Expense.partnerName`, `WorkJournal.partnerName`~~ | **해소(V97)** | 문자열은 그대로 두고, 이름이 마스터와 **정확히 일치할 때만** `partner_id` 를 채운다. 마스터에 없는 상대에게도 돈은 나가므로 자유입력을 없애지 않았다 |
| ~~`BoardPost.author`, `Notice.author`, `WorkPost.writer`~~ | **해소(V96)** | 셋 다 로그인 사용자명이 그대로 들어가므로 `users(username)` FK 로 묶었다 |
| `QualityInspection.inspector`, `WorkResult.worker`, `BusinessPartner.manager`, `Project.manager` | `String` (유지) | **FK 를 걸지 않는다.** 외부 검사원·현장 작업자·거래처 담당자·PM 은 우리 계정이 아닐 수 있다. '이름을 담는 칸'과 '사용자를 가리키는 칸'은 다르다 |
| ~~`LotStatus` enum~~ | **해소(V97 무관·코드만)** | 상태는 저장하지 않고 `stockQty`+`held` 에서 파생한다. 응답 DTO 가 따로 한글 문자열을 지어내던 것을 이 enum 하나로 모았다 |
