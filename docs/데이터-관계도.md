...
2. 백엔드 재기동 → Flyway가 적용하고, `validate`가 엔티티와 스키마 일치를 확인한다.
3. `node qa/qa.mjs`로 회귀를 확인한다.
4. 새 관계(FK)는 **nullable로 추가**하고 기존 행은 `UPDATE ... FROM`으로 백필한다. 그래야 기존 데이터가 살아남는다.

### 왜 이렇게 바뀌었나 (과거 사고)

예전에는 `ddl-auto: update`였는데, **기존 행이 있는 테이블에는 기본값 없는 `NOT NULL` 컬럼이 추가되지 않습니다.**
...
계속 500을 내고 있었으며, 화면에는 그냥 빈 목록으로 보였습니다. Flyway 도입으로 이 함정은 사라졌습니다.

### DB 초기화

```bash
docker compose down -v && docker compose up -d   # ⚠️ 데이터 전부 삭제
cd backend && ./mvnw spring-boot:run             # Flyway가 V1~V3 실행 + 기본 시드
node qa/qa.mjs                                   # 검증
```
