...
| 재고 I › 쇼핑몰관리 | 10 / 11 | MallOrder+MallPage가 주문관리·진행단계·확인·취소·판매전환·상품생성설정·배송/반품/교환(V112·V113) 커버. **품목코드연결=MallItemMappingPage 신규(V114, 수집 자동연결). 잔여: 몰계정 기초등록(오픈API 인증 연동 필요 — 아래 8장)** |
...
| 그룹웨어 › 공용메일 | 8 / 9 | MailPage가 수신/발신/공용·수신확인·메일쓰기·기본함 커버. **임시보관함(초안)·지운함(소프트삭제)=MailPage 신규(draft·deleted_at+V111). 잔여: 스팸(외부메일 연동 없음 — 스코프 외)** |
...
| **합계** | **≈241 / 340** | 2026-07-20 구매·영업 파이프라인 + 재고실사 + 품질/A/S현황 + 매출계획(풀스택) + 로트재고비교 구축, 생산/외주·회계기초·수출 포함 오탐 대량 정정 반영. 2026-07-27 특별단가등록(회계 I 기초등록 13/13) + 카드사·결제대행사(재고 I 기초등록 7/7) + Self-Customizing 5/5 오탐 정정 + 메일 임시보관함·지운함(공용메일 8/9) + 쇼핑몰 배송/반품/교환·품목코드연결(쇼핑몰 10/11) |
...
## 8. 미구현(⬜) 사유 정리 (2026-07-27 기준)

DOM 근거가 있는 실제 구현 대상은 사실상 소진됐다. 남은 ⬜ 20종은 아래 사유로 **의도적으로 두거나 구조적으로 불가**하다.
새로 착수하려면 이 표의 전제(외부 연동·모델 확장·정책 결정)를 먼저 풀어야 한다.

### 8.1 외부 시스템 연동이 있어야 의미가 생김 (연동 없이는 빈 껍데기/가짜 컨트롤)
- `그룹웨어_업무관리_E077100` **ECDrive** — 이카운트 클라우드 드라이브(외부 파일저장 서비스). 저장소 연동이 본체.
- `그룹웨어_업무관리_E200162` **WORK** — 이카운트 WORK(외부 협업 플랫폼) 연동.
- `그룹웨어_공용메일_E077005` **스팸 메일함** — 외부 메일서버(IMAP)·스팸필터 연동 없이는 분류 소스가 없음(사내메일만 다루는 우리 모델 밖).
- `데이터센터_데이터내보내기_E040231` **의료기기공급내역보고** — 심평원 UDI 공급내역 대외 보고(규제기관 전송 규격·인증서). 외부 제출 채널이 본체.
- `재고_I_쇼핑몰관리_C000664` **몰 계정 기초등록** — 쇼핑몰 오픈API 인증(키·토큰) 저장이 목적. 연동이 없으면 저장해도 쓰이는 곳이 없어(수집은 수동 입력) 빈 자격증명 마스터가 됨. 오픈API 수집이 붙을 때 함께 구현.

### 8.2 우리 전표/데이터 모델 밖 (구조적)
- `재고_I_영업관리_E040253` **판매입력 II** — 사용자정의 문자/숫자/일자/코드 형식필드가 대량. 우리 판매전표 스키마로는 흡수 불가(EAV형 커스텀필드 엔진이 선행).
- `데이터센터_데이터수집_E100000` **수집데이터등록** — 수집 소스가 코드에 하드코딩(DataCollectPage 9종). 동적 소스 등록은 소스 레지스트리+플러그인 구조가 선행.

### 8.3 이미 다른 화면이 커버 / 중복 (의도적 제외)
- `재고_I_출력물_E040703` **채권/채무현황** · `재고_I_출력물_E040721` **채권현황** — 거래처관리대장(LedgerPage)·채무관리(PayablePage)·월별채권채무(MonthlyArApPage)와 중복.
- `재고_I_생산_외주_E040416` **생산입고 III** — 생산입고 I/II(E040406/407)와 동일 기능 중복.
- `재고_I_출력물_E010851` **쪽지수발신내역** — 쪽지(단문 IM) 엔티티 없음. 사내 커뮤니케이션은 사내메일(MailPage)로 대체.

### 8.4 개별 화면이 아님 (출력물 목차 카테고리 노드 / 플랫폼 허브)
- `재고_I_출력물_C000095` 영업관리현황 · `C000096` 구매관리현황 · `C000648` 생산/외주현황 — 하위 현황들을 묶은 **목차 카테고리 노드**(개별 화면 아님, 각 하위 현황은 이미 구현).
- `재고_I_출력물_C000663` **커뮤니케이션센터** · `재고_I_출력물_E040730` **증빙센터** — 이카운트 플랫폼 차원의 문서/증빙 저장 허브. 우리 앱 범위 밖.
- `재고_I_출력물_E040607` **잔량재집계** — 배치성 재집계 유틸. 우리는 재고·잔량을 조회 시 실시간 집계하므로 별도 재집계 배치가 불필요(설계 차이).

### 8.5 그룹웨어 부가 (범위/정책 결정 필요)
- `그룹웨어_공유정보_C000698` **사내관리** — 사내 규정·자료 관리 포털. 담을 범위가 불명확(게시판/공지로 부분 대체 중). 요구 정의가 선행.
- `그룹웨어_공유정보_E070203` **조건별검색** — 저장된 조건 검색(횡단 기능). 현재 각 목록화면의 검색 패널로 대체.

> 위 20종은 화면 추가가 아니라 **연동·모델확장·정책결정**이 선행 과제다. 그 전제가 갖춰지면 5장 레시피로 착수한다.
...
| ⬜ | `그룹웨어_공용메일_E077005` | 스팸 메일함 (외부메일 연동 없음 — 스코프 외) |
