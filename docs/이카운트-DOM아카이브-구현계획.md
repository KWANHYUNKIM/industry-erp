...
### 완료 로그 (2026-08-25 19차, 판매현황 [현황|집계] — 2단계 그룹화)

원본 판매현황 맨 위에는 `메뉴 [현황][집계]` 토글이 있다. **집계로 바꾸면 `구분` 줄이
`라인별 ▾` 하나에서 `집계조건1` + `집계조건2` 두 줄로 바뀐다** — 즉 **2단계 그룹화**다.
우리는 라인 목록(현황)만 있었다.

원본 [항목추가] 팝업에서 확인한 집계 기준:
**일별 · 주차별 · 월별 · 분기별 · 반기별 · 연별 · 담당자별 · 창고별 · 거래유형별 · 거래처별 ·
프로젝트별 · 전표별 · 관리항목별 · 품목별 · 라인별**

- ✅ 이 중 **우리 데이터로 실제 묶을 수 있는 14종**을 넣었다(라인별은 곧 현황 모드라 뺐다).
  주차·분기·반기는 일자에서 파생한다(주는 **월요일 시작** — `EcPeriodPicks` 와 같은 셈법).
- ✅ 값이 없으면 `(없음)`·`(미지정)` 으로 묶는다 — 빈칸이 흩어지면 못 읽는다.
- ✅ 집계 표는 그룹키(1~2열) + 건수·수량·공급가액·부가세·합계, **금액 큰 그룹이 위로**.
- ⚠️ **NUL 바이트를 낼 뻔했다** — 그룹 키를 잇는 구분자로 널문자 이스케이프를 쓰려다 파일에
  실제 NUL 이 박혀 소스가 바이너리로 취급됐다. 눈에 안 보이는 사고라 `grep` 이
  "Binary file matches" 를 뱉고 나서야 알았다. 보이는 문자(U+241F)로 바꿨다.
- ✅ **검증** — `npm run typecheck` · **QA 438/438** · 브라우저에서
  현황 **124라인** → 품목별 집계 **1그룹(수량 186 · 공급가액 1,860,000)** ·
  `월별` → `2026-03` · `월별+품목별` 2단계 → `2026-03 | QA완제품` ·
  **열 수 th/td/tfoot 8/8/8** (12차에서 만든 합계행 규칙대로 맞춤) · 콘솔 오류 0.

---

### 완료 로그 (2026-08-25 20차, 구매현황 집계 + 집계 규칙을 공용으로)

원본 **구매현황(`E040305`)도 `[현황|집계]` 토글과 같은 빠른선택 묶음**을 쓴다(실측 확인).
두 화면이 같은 규칙을 쓰는데 19차에서 판매현황 안에 계산을 인라인으로 넣어 뒀다 —
**같은 계산을 두 곳에 적으면 반드시 어긋난다**(부가세 배분을 `VatAllocator` 로 모은 것과 같은 이유).

- ✅ **`utils/statusAggregate.ts` 신설** — `GROUP_KEYS` · `weekOfYear` · `groupValue` · `aggregate`.
  화면은 `AggregatableRow` 모양만 맞추면 된다(Row 필드명이 달라도 매핑해서 넘긴다).
  - 주는 **월요일 시작** — `EcPeriodPicks` 와 같은 셈법이어야 한다.
    한쪽만 일요일 시작이면 같은 날이 다른 주가 된다.
  - 그룹 키 구분자는 **보이는 기호**를 쓴다. 19차에서 널문자를 쓰려다 소스에 NUL 이 박혔다.
- ✅ 판매현황을 공용 모듈로 옮기고, 구매현황에 같은 토글·집계 표를 붙였다.
- ℹ️ 구매 라인에는 품목의 관리항목이 실려 있지 않아 `관리항목별` 은 `(없음)` 으로 묶인다.
  판매현황은 품목 마스터에서 파생해 넘긴다 — **화면이 가진 것만큼만 넘긴다.**
- ✅ **검증** — `npm run typecheck` · **QA 438/438** · 브라우저에서
  판매현황이 **옮긴 뒤에도 그대로**(126라인 → 1그룹, 열 7/7/7) ·
  구매현황 126라인 → 품목별 1그룹(수량 2,205 · 공급가액 2,583,000) ·
  `창고별+거래처별` 2단계 → `QA창고 | QA매입처` · 열 **8/8/8** · 콘솔 오류 0.

---

### 완료 로그 (2026-08-25 21차, 일정관리 — 달력이 아예 없었다)

원본 일정관리(공유정보 › 사내관리)는 **[월 캘린더 | 일정 목록] 2분할**이다.
왼쪽에 달력과 `내 캘린더`·`다른 캘린더`, 오른쪽에 `일자(요일) · 시작시간 · 종료시간 · 참석자성명` 표.
**우리는 목록만 있었다** — "이번 달 언제 뭐가 있나"를 한눈에 볼 수 없었다.
일정 화면에서 달력은 장식이 아니라 본체다.

- ✅ **`components/EcMonthCalendar.tsx` 신설** — 월 이동 · 오늘 표시 · **일정 있는 날 점** ·
  날짜를 누르면 그날만, 같은 날을 다시 누르면 전체. 다른 일정 화면도 쓸 수 있게 컴포넌트로 뒀다.
  - 날짜는 **로컬 기준**(`EcPeriodPicks.ymd` 재사용 — `toISOString()` 은 UTC 로 밀린다).
  - 달력 격자는 원본과 같이 **일요일 시작**이다. (기간 빠른선택의 '주'가 월요일 시작인 것과 별개다 —
    달력 격자와 주 단위 집계는 다른 개념이다.)
- ✅ 컬럼을 원본에 맞췄다 — `일자(요일)` · 시작시간 · **참석자성명**(V118 로 이미 있던 필드가 여기서 쓰인다).
- ⚠️ **시험하다 진짜 UX 문제를 잡았다** — 달력 첫 줄의 회색 칸(이전 달 날짜)을 누르면
  **달력은 8월인데 7월 일정이 뜬다.** 내 테스트가 8월 27일 대신 7월 27일을 집어 0건이 나왔고,
  안내줄을 보고서야 알았다. **내가 헷갈렸으면 사용자도 헷갈린다** —
  다른 달 칸을 누르면 달력도 그 달로 옮기도록 고쳤다.
- ✅ **검증** — `npm run typecheck` · **QA 438/438** · 브라우저에서 일정 2건을 만들어
  달력에 **점이 25·27일에 찍히고**, 8/27 을 누르면 그날 1건만, 이전 달 칸을 누르면 **달력이 7월로 이동**하는 것
  확인 후 시험 일정 삭제 · 콘솔 오류 0.
...
## 2026-08-26 · 재고현황 그룹 실측 대조 (살아 있는 원본 메뉴트리 기준)

원본 `출력물 > 재고현황` 그룹은 아홉 화면이다. 우리 것과 한 줄씩 맞춰 봤다.

| 원본 | prgId | 우리 |
|--|--|--|
| 재고현황 | E040701 | ✅ `CurrentStockPage` |
| 창고별재고현황 | E040711 | ✅ **신규** `WarehouseStockPage` (종/횡) |
| 관계별재고현황 | E040728 | ⬜ **보류** — 품목관계(대체품·세트) 마스터가 우리에게 없다. 화면 하나가 아니라 품목등록까지 건드리는 일이라 따로 잡는다 |
| 다규격별재고현황 | E040720 | ⬜ **보류** — 다규격(색상·사이즈 변형) 마스터 없음. 위와 같은 이유 |
| BOM환산재고현황 | E040726 | ✅ **신규** `BomStockPage` |
| 재고잔량분석표 | E040727 | ✅ `StockAnalysisPage` |
| 재고수불부 | E040702 | ✅ `StockLedgerPage` |
| 재고변동표 | E040719 | ✅ `StockMovementPage` (집계/일별/월별) |
| 일보 | E040708 | ✅ `DailyReportPage` — 단일 일자였던 것을 **구간 + 일자별 요약**으로 고침 |

우리 쪽에만 있는 `입출고`(입출고 등록 폼)·`잔량재집계`(원본은 기타이동 그룹)는 그대로 둔다.

### 영업·구매 관리현황 그룹에서 드러난 것

원본 `구매관리현황`에 있는 **거래처별채무·채무현황·거래처관리대장1(채무)·월별채무증감내역**이
우리 메뉴에 통째로 없었다. 화면 코드는 이미 채권/채무 양쪽을 다루고 있었고(ArApStatusPage 의
`PAYABLE`, MonthlyArApPage 의 `AP`) **가는 길만 없었다.** 경로를 만들고 원본 순서대로 넣었다.

아직 없는 것:

- **미판매현황** — 판매 전환이 안 된 수주를 보는 화면. 미주문/미출하는 있는데 이것만 없다
- **매출/매입(세금)계산서조회(재고)·현황(재고)** — 우리 세금계산서는 회계 쪽에만 있다.
  재고 모듈에서 전표 기준으로 보는 판이 따로 있어야 원본과 같아진다


---

## 2026-08-26 (이어서) · 출력물 전 그룹 대조 + 이익 계산 결함

### 새로 만든 화면

| 원본 | prgId | 우리 |
|--|--|--|
| 미판매현황 | E040212 | `UnsoldStatusPage` `/sales/unsold` — 백엔드 `GET /api/sales-orders/unsold` 신설 |
| 창고이동현황 | E040505 | `TransferStatusPage` `/inventory/transfer-status` |
| 자가사용·불량처리·대체사용·폐기·재고조정현황 | E040506/509/510/511/608 | `StockMoveStatusPage` — `kind` 하나로 다섯 화면 |
| 재고실사현황 | E040615 | `StocktakeStatusPage` `/inventory/stocktake-status` |
| 일별재고현황 | E040807 | `DailyStockPage` `/inventory/daily-stock` |
| 생산입고/소모현황 I | E040415 | `ProductionIssueStatusPage` `/production/receipt-issue-status` |

### 이익 계산이 틀려 있었다

`/api/profit/daily`·`/monthly` 는 **그날(그달)의 매입액**을 원가로 놨다. 자재를 몰아 사들인
날은 잘 팔았어도 적자로 찍힌다. 이익은 **판 물건의 원가**로 재야 한다.

- 일별·월별이익현황을 판매 라인 기준으로 다시 만들었다: 이익 = 판매액 − (판매수량 × 원가단가)
- 원가 기준 [월별원가|최종구매가|품목단가]. 원본의 **선입선출은 뺐다** — 입고 레이어를
  남기지 않아 계산할 수가 없다. 있는 척하고 다른 값을 보여 주는 것보다 없는 편이 낫다
- 원가를 모르는 라인은 0 이 아니라 `—`. 이익률은 원가를 아는 라인의 판매액으로 낸다
- 이름이 '이익'인데 매출−매입을 주는 엔드포인트는 다음 사람이 또 쓰게 되므로 지웠다.
  되살아나면 QA 가 잡는다(404 단언)
- **다른 데는 멀쩡하다**: 품목별원가/이익·손익요약은 판매수량 × 원가로 제대로 계산하고,
  프로젝트별 손익은 매출−매입−비용이지만 프로젝트는 산 것이 곧 원가라 맞다.
  경영자보고서는 같은 계산이지만 화면이 '추정치'라고 말하고 있어 그대로 뒀다

곁다리: 없는 API 경로가 404 가 아니라 **500** 을 내면서 `"No static resource api/..."` 라는
내부 사정까지 실어 보내고 있었다. 핸들러를 추가해 404 로 돌린다.

### 조건 판 스윕 (10개 화면)

날짜 칸 두 개짜리 한 줄이던 현황 화면들을 실측한 `EcStatusPanel` 로 바꿨다 —
계정별원장 · 회계전표조회 · 프로젝트별손익 · 불량률파악보고서 · 지각현황 · 경영자보고서 ·
합계잔액시산표 · 손익계산서 · 재무상태표 · (세금)계산서.
재무상태표만 한 시점 잔액이라 기준일 하나(single)로 뒀다.

**손대지 않기로 한 것들** — 이미 원본을 맞춘 것이라 바꾸면 멀어진다:
미구매현황 · 발주서현황 · 품질검사현황(원본의 접이식 Search 패널),
채권/채무현황(화면별 빠른선택 금일·전월말일·당월말일 + 코드도움),
판매조회·구매조회·출하조회(조회 계열 Search 패널).

### 브라우저 확인이 밀린 화면

확장이 붙은 Chrome 이 **다른 기기 것**이라 렌더 확인을 못 한 채 커밋한 것들이다.
숫자는 API 로 같은 계산을 돌려 맞췄고 표 셀 수는 손으로 대조했지만, 화면을 직접 연 적은 없다.
**원본에 다시 접근할 수 있게 되면 이 목록부터 열어 볼 것.**

- 월별이익현황
- 재고실사현황 (E040615 조건 판도 미실측 — 형제 화면 실측 모양을 따랐다)
- 생산입고/소모현황 I (E040415 조건 판도 미실측)
- 조건 판 스윕 10종

### 여전히 남은 원본 화면

- 관계별재고현황(E040728) · 다규격별재고현황(E040720) — 품목관계·다규격 **마스터가 없다**.
  화면 하나가 아니라 품목등록까지 건드리는 일이라 따로 잡는다
- 매출/매입(세금)계산서조회·현황(재고) — 원본이 **권한없음**이라 실측 불가
- 거래처관리대장 I / II — 원본은 둘인데 우리는 하나. 무엇이 다른지 실측이 필요하다


---

## 2026-08-26 · 목록 API 응답 측정 (버그는 못 찾음)

N+1 이 있는지 보려고 목록 엔드포인트 30개의 응답 시간을 쟀다. **N+1 은 없다.**
가장 느린 `/journals`(전체 연도)도 234ms 이고, 그건 5,125건 3.2MB 라는 자료량 때문이다.
기간 조건도 제대로 먹는다(8월만 296건, 하루만 0건).

대신 <b>구조적인 것</b>이 하나 드러났다. 지금 문제는 아니지만 자라는 종류다.

| 경로 | ms | 크기 | 건수 | 통째로 받는 화면 |
|--|--|--|--|--|
| `/journals` (연) | 234 | 3.2MB | 5,125 | 1 |
| `/sales` | 29 | 390KB | 512 | **18** |
| `/purchases` | 21 | 206KB | 296 | **20** |
| `/sales-orders` | 23 | 169KB | 351 | 3 |
| `/purchase-orders` | 20 | 191KB | 301 | 6 |

- 페이지네이션이 있는 엔드포인트는 앱 전체에 **`/stock/transactions` 하나뿐**이다.
- 현황 화면 대부분이 전체 목록을 받아 **프론트에서 거른다.** 지금은 수백 건이라 괜찮지만,
  거래가 쌓이면 화면을 열 때마다 수 MB 를 받게 된다.

**고치지 않았다.** 페이지네이션을 넣으려면 API 계약과 38개 화면의 거르는 방식을 같이 바꿔야 하고,
그건 한 번의 반복으로 할 일도, 측정 없이 결정할 일도 아니다. 판단에 필요한 숫자만 남긴다.


---

## 2026-08-26 · 테넌트 온보딩 점검 (남은 항목은 전부 정상)

새로 만든 회사가 비어 있어 못 쓰는 것이 더 있는지 훑었다. **두 건은 실제 결함이라 고쳤고**
(계정과목 0개, 회사정보 상호 없음 — 각각 별도 커밋), 나머지는 정상이었다.
다음에 같은 곳을 다시 파지 않도록 남긴다.

| 테넌트에 없는 것 | 본사 | 판정 |
|--|--|--|
| `accounts` 계정과목 | 37 | **결함 → 고침.** 코드가 108·135·251·255·801·254 를 코드값으로 찾아 쓴다 |
| `company_info` 상호 | 0 | **결함 → 고침.** 인쇄물 공급자란이 "(회사정보 미등록)" 이 된다 |
| `preference` 환경설정 | 1 | 정상 — 행이 없으면 기본값으로 응답한다(`id: null`, fiscalStart 01 …) |
| `security_policy` | 1 | 정상 — 위와 같이 기본값 |
| `order_types` · `order_stages` | 5·5 | 정상 — `OrderMetaInitializer` 가 만드는 <b>데모</b> 자료다 |
| `approval_form_templates` | 23 | 정상 — 시드가 아니라 앱에서 만든 사용자 자료 |
| `common_codes` · `currencies` | 15·4 | 정상 — 카드사·통화 모두 사용자가 등록하는 자료 |

그 밖에 확인하고 손대지 않은 것:

- 인쇄 네 화면(거래명세서·견적서·발주서·매입명세서)은 회사정보가 없을 때
  "(회사정보 미등록)" 으로 대체한다 — 빈칸으로 나가지 않는다
- 로그인 응답이 `companyCode`·`companyName` 을 주고 앱바가 그대로 보여 준다 —
  어느 회사에서 일하는지 헷갈릴 여지가 없다
- 채번은 회사별로 독립이다(같은 날 두 회사가 각자 `SO-…-0001`)
