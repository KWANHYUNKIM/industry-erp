...
**(c) 죽은 테이블 — `order_stages`, `order_types`** ☠️

`OrderMetaInitializer`가 시드만 넣고 **읽는 코드가 어디에도 없습니다.** `SalesOrder`에는 stage/type 필드 자체가 존재하지 않습니다.

둘 중 하나를 택해야 합니다.
- 이카운트의 수주 "진행단계 / 거래유형"을 구현할 계획이면 → `sales_orders.stage_id`, `sales_orders.order_type_id` FK 추가
- 아니면 → 테이블과 initializer 삭제

지금 상태는 "시드는 돌지만 아무 의미 없는 데이터"입니다.
...
| `V2` | FK 인덱스 55개 | 낮음 | 🔄 진행 중 (다른 세션) |
| `V3` | 삭제 규칙: 소유 관계 7개 → `CASCADE` | 낮음 | 문제 2 |
| `V4` | 삭제 규칙: 마스터 참조 → `RESTRICT` | 중간 | 하드 삭제 코드 있으면 먼저 제거 |
| `V5` | `created_by_id` 15개 테이블 추가 (nullable) + 백필 | **높음** | 백필 결과 검증 필수 |
| `V6` | 백필 성공 시에만 `NOT NULL` + FK | 높음 | V5와 분리할 것 |
| `V7` | `author`/`writer`/`owner`/`manager`/`worker` 7곳 | 중간 | `BusinessPartner.manager` 결정 후 |
| `V8` | `work_results.process` 문자열 컬럼 제거 | 낮음 | 엔티티 수정 동반 |
| `V9` | `order_stages`/`order_types` — 연결 또는 삭제 | 낮음 | 제품 결정 필요 |

`V5`/`V6`을 분리하는 이유: 백필이 실패해도 마이그레이션이 절반만 적용된 채로 남지 않게 하기 위함입니다.
...
| 3 | `WorkResult.worker` 4명이 `users`에 없다. 어떻게 할 것인가? | ❓ 사람이 결정 (§2 문제 3) |
| 4 | `order_stages` / `order_types`를 살릴 것인가, 지울 것인가? | ❓ 제품 결정 (§2 문제 4c) |
| 5 | 스키마 변경 전 백업을 자동화할 것인가? | ❓ 15:38 사고 재발 방지 (§4.0) |
