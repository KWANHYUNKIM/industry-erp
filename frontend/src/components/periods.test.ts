...
test('최근30일(+1개월) — 앞으로 한 달까지 본다', () => {
  // 작업지시서작업처리는 앞으로 할 일을 고르는 화면이라 미래가 들어간다.
  // 오늘까지만 보면 아직 안 온 납기의 작업지시가 빠져 '할 일이 없다' 로 보인다.
  assert.deepEqual(periodOf('최근30일(+1개월)', new Date(2026, 6, 15)),
    { from: '2026-06-15', to: '2026-08-15' })
})

test('최근30일(+1개월) — 달을 넘어가도 같은 날로 잡는다', () => {
