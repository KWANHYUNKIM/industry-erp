...
/**
 * 채권현황(E040721) — 조회 묶음 뒤에 <b>[말일]</b>이 붙는다(2026-09-02 원본 실측).
 * 기준일자가 <b>한 날짜</b>인 화면이라 고른 구간의 끝날만 쓴다. 받을 돈은 월말에 끊어
 * 보는 일이 잦아서 그 버튼이 따로 있다.
 */
export const BALANCE_PICKS = [...BASE_PICKS, '말일'] as const

/**
 * 거래처별채권(E040214) · 거래처별채무(E040309) — 조회 묶음 뒤에 <b>[종료일]·[전월+금월]</b>
 * 둘이 붙는다(2026-09-07 원본 실측). 예전에는 STATUS_PICKS(판매현황 묶음)를 그대로 써서
 * <b>[종료일]이 빠져 있었다</b>. 이 화면은 기준일자가 <b>구간</b>이라 끝날을 회계 종료일로
 * 맞추는 버튼이 따로 있다 — 채권현황의 [말일]과는 다른 것이다(그쪽은 한 날짜 화면이다).
 */
export const PARTNER_LEDGER_PICKS = [...BASE_PICKS, '종료일', '전월+금월'] as const
