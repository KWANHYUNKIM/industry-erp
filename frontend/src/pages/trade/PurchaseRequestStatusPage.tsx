 ...
 */
/**
 * <b>단가요청현황의 [구분]은 이름만 같고 다른 것을 고른다</b>(2026-09-09 E040325 실측).
 * 발주요청현황·발주계획현황의 [구분]은 <b>내역/집계</b> 인데, 단가요청현황은
 * <b>[요청단가]·[수취단가]·[확정단가]</b> 세 알이다(기본 요청단가) — 격자의 단가 세 칸과
 * 짝이 맞는 축이다. 우리 발주 줄은 단가가 하나뿐이라(덮어쓴다) 가를 축이 없어
 * 이 화면도 내역/집계를 그대로 쓴다. 단가를 셋으로 나눌 때 같이 만들 일이다.
 */
      ...
      <table className="w-full text-left">
        <thead>
          <tr>
            ...
            */}
            {/*
              <b>단가요청현황(E040325) 2026-09-09 원본 격자 실측</b> —
              [일자No. · 거래처명 · 품목명[규격] · 수량 · 단가(요청단가) · 단가(수취단가) ·
              단가(확정단가)] 일곱 칸이다. 겸하는 발주요청현황과 <b>같은 자리를 다르게 부른다</b>:
              하이픈 없는 <b>[일자No.]</b> 이고 규격을 <b>대괄호</b>로 감싼다.
