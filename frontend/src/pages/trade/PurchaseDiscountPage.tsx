...
export default function PurchaseDiscountPage() {
   ...
   * 같은 매입처가 여러 줄로 흩어진다 — 어디서 할인이 큰지 눈으로 더해야 했다.
   */
  const SUBTOTALS = ['매입처', '품목'] as const
      ...
      */}
      ...
      {view === '표' && shown.length > 0 && (() => {
        const groups = subtotalBy(shown, (r) => (subtotal === '품목' ? r.itemName : r.partnerName), {
          qty: (r) => r.qty, buy: (r) => r.buyPrice, disc: (r) => r.discountAmount,
        })
        return (
          <>
            ...
            <table className="w-full text-left">
              <thead><tr>
                ...
                <th style={{ width: 140, textAlign: 'right' }}>매입가</th>
