  ...
  return (
    <form ref={formRef} onSubmit={submit}>
      ...
      >
        ...
        {/*
          원본 판매입력 II(ESD006M)에는 "금액조정항목명/공급가액/부가세/합계/외화금액/원화금액"
          표가 **없다**. 합계는 그리드 하단 합계행이 지고, 우리 표는 그것과 중복이었다.
          원본 구조(헤더폼 → 툴바 → 그리드 → 합계행 → 푸터)에 맞춰 걷어낸다.
          다만 외화 전표일 때의 환산액은 합계행이 못 보여 주므로 한 줄로 남긴다.

          예전에는 이 자리에 "아카이브 DOM 전체에 '금액조정'·'외화금액'·'원화금액' 0건" 이라고
          적혀 있었다. **세어 보니 사실이 아니다** — 금액조정항목명 3건(전부 ESD066M),
          외화금액 2건(EBZ061R), 원화금액만 0건이다. 결론은 그대로다: 그 표는 **다른 화면** 것이고
          이 화면(ESD006M)에는 없다. 근거를 지금 사실로 고쳐 적는다.
        */}
        {foreign && rate > 0 && (
