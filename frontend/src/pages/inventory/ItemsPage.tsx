      ...
      <Modal open={showForm} title="품목등록" onClose={() => setShowForm(false)}>{(
        <form onSubmit={submit} style={{ marginTop: 8, marginBottom: 8, border: '1px solid var(--ec-border)', background: '#fff', padding: 14 }}>
          ...
          {/*
            ...
            <b>원가 탭은 만들지 않는다</b> — 다만 여기 적혀 있던 '넷이 다 없다' 는 틀렸다.
            <b>외주비는 있다</b>: <code>subcontractPrice</code> 가 바로 아래 [단가] 탭에
            <b>[외주비단가]</b> 로 떠 있다. 없는 것은 재료비·노무비·경비 셋이고,
            하나 때문에 탭을 따로 두면 그게 빈 탭이 된다. 부가정보에 해당하는
