 ...
 */
{
  ...
  const NO_OPTION = new Map([
    /*
     * <b>이유가 썩어 있었다.</b> '거래처에 계층이 없다' 고 적어 뒀는데 대표거래처는 그 사이
     * 생겼고(BusinessPartner.parent · 거래처리스트 [관계설정]), 거래처관리대장은 이미
     * <b>[대표거래처로 합산]</b> 체크 하나로 대표 단위와 개별 단위를 오간다.
     * 같은 선택을 라디오로 한 번 더 그리면 <b>서로 어긋날 수 있는 칸이 둘</b>이 된다.
     * 거래처관리대장 II 는 거래처등록 화면이라 합칠 잔액 자체가 없다.
     */
    ['거래처관계기준', '[대표거래처로 합산] 체크 하나가 곧 이 선택이다 — 같은 것을 두 번 그리지 않는다'],
    ['개별거래처기준', '위와 같음 — 그 체크를 끄면 개별 거래처 단위다'],
