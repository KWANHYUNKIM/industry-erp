 ...
 */
{
  ...
  const FILE_OF = new Map([
     ...
     * 2026-09-08 에 둘 다 열어 견주었다. 조건 수는 열셋으로 같은데 판이 다르다:
     * 이쪽에만 <b>[구분](채권★·채무·채권/채무)</b> 과 <b>[양식]</b> 이 있고,
     * 체크는 낱개가 아니라 <b>[기타]</b> 안에 묶여 있다.
 ...
 */
{
  ...
  const NO_FIELD_ON = new Map([
    ...
    /*
     * [규격]에는 근거를 못 달았다 — 같은 파일의 <b>UnshippedLineResponse</b> 가 spec 을
     * 싣고 있어서 'String spec 이 없다' 는 파일 단위 근거가 성립하지 않는다.
     * 없는 것은 <b>OrderLineResponse</b> 쪽이다.
     */
    ['주문서현황|규격', '<code>OrderLineResponse</code> 가 규격을 안 싣는다(같은 파일의 미출하 응답은 싣는다)'],
