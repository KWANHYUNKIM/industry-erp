 ...
 */
{
   ...
   *
   * <p>아직 안 잰 사본의 조건 수:
   *   생산불출조회 5 · 생산입고조회 6 · 작업지시서조회 6 · 매출계획조회 7 · 설문조사조회 7 ·
   *   거래이력조회 8 · 작업내역조회 8 · 근태조회 9 · 결제내역조회 10 ·
   *   프로젝트계획조회 10 · A/S접수조회 11
   *
  ...
  const NO_FIELD_ON = new Map([
     ...
     */
    /*
     * <b>생산입고조회(C000032)</b> — 2026-09-08 원본 실측, <b>서른둘</b>이다(사본은 넷).
     ...
     */
    /*
     * <b>A/S접수조회(C000091)</b> — 2026-09-08 원본 실측, <b>서른하나</b>다(사본은 열하나).
    ...
    ['미판매현황|거래유형', '과세·면세를 수주에서 정하지 않는다 — 판매 전표에서 정한다'],
    ...
    ['미주문현황|거래유형', '과세·면세를 견적에서 정하지 않는다 — 판매 전표에서 정한다'],
    ['미주문현황|참조', '견적은 시작점이라 근거전표를 달지 않는다'],
    ['미주문현황|결제조건', '거래처에도 견적에도 결제조건 칸이 없다'],
    ['미주문현황|규격', '<code>QuoteLine</code> 이 규격을 안 싣는다'],
    ...
    ['주문서현황|거래유형', '과세·면세를 수주에서 정하지 않는다 — 판매 전표에서 정한다'],
     ...
     * (사본은 여덟). 접힌 줄은 없다. 조회 쪽(C000033, 스물아홉)과 이름이 거의 같은데
 ...
 *
 * <p>한 가지 더: <b>조회 화면이라고 다 C 코드인 것은 아니다.</b>
 * 판매조회 C000030 · 생산입고조회 C000032 · 창고이동조회 C000033 · 발주서조회 C000077 ·
 * 출하지시서조회 C000120 은 C 인데, <b>생산불출조회는 E040405</b> 다(둘 다 주소창에서 직접 읽었다).
 * 모양으로 넘겨짚지 말고 화면을 열어 읽는다.
 *
 * <p><b>지어내지 않고 비워 둔 자리:</b> A/S접수조회는 지도에 <code>C000091</code> 이라
 * 박혀 있었는데, 메뉴에서 <code>C000091</code> 은 <b>A/S관리 모듈</b> 이었다.
 * 맞는 코드를 아직 모르므로 그 줄을 <b>지웠다</b> — 틀린 값을 남겨 두면 다음 사람이 믿는다.
 */
{
