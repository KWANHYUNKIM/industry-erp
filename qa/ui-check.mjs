 ...
 */
{
  ...
  const NO_FIELD_ON = new Map([
     ...
     */
    ['판매조회|출하창고', '창고로 거르는 칸을 아직 안 만들었다(목록에는 [창고] 열이 있다)'],
    ['구매조회|입고창고', '위와 같음'],
    ['판매조회|통화', '통화·환율을 아직 안 만들었다 — 전표가 다 원화다'],
    ['구매조회|통화', '위와 같음'],
    ['판매조회|프로젝트', '프로젝트로 거르는 칸을 아직 안 만들었다(구매 목록에는 [프로젝트명] 열이 있다)'],
    ['구매조회|프로젝트', '위와 같음'],
    ['판매조회|거래유형', '거래유형으로 거르는 칸을 아직 안 만들었다(목록에는 [거래유형명] 열이 있다)'],
    ['구매조회|거래유형', '위와 같음'],
     ...
     */
    ['거래처관리대장 II|외화거래처', '거래처에 통화를 두지 않는다 — 외화·환율을 아직 안 만들었다'],
    ['구매일괄회계반영|담당자', '전표 담당자로 거르는 칸을 아직 안 만들었다 — 구매전표에 담당 사원(employeeId)은 있다'],
    ['구매일괄회계반영|구매No.', '전표번호로 거르는 칸을 아직 안 만들었다'],
    /*
     * [구매구분]·[거래구분]은 사본에 고를 값이 안 담겨 있다(빈 선택 상자로 찍혔다).
     * 무엇을 가르는 조건인지 모르는 채로 칸만 만들면 뜻 없는 조건이 하나 는다.
     * 원본에서 값을 확인한 뒤에 만든다.
     */
    ['구매일괄회계반영|구매구분', '사본에 고를 값이 안 담겨 무엇을 가르는지 알 수 없다'],
    ['구매단가일괄변경|구매구분', '위와 같음'],
    ['판매단가일괄변경|거래구분', '위와 같음'],
    ['기안서통합관리|프로젝트', '기안서를 프로젝트에 잇지 않는다 — 아직 안 만들었다'],
    ['기안서통합관리|라벨', '기안서 라벨(꼬리표)을 아직 안 만들었다'],
    /*
     * 생산 쪽 [담당자]. 작업내역입력은 머리에 담당자가 있는데(이번에 이름을 맞췄다),
     * 생산입고·불출·작업처리 전표에는 사람을 안 남긴다. 남기려면 작업지시처럼
     * employeeId 컬럼과 마이그레이션(본사·테넌트 양쪽)이 필요하다.
     */
    ['생산입고I-BOM기준소모|담당자', '생산입고 전표에 담당자를 남기지 않는다 — 컬럼부터 만들어야 한다'],
    ['생산입고II-소모품목 선택|담당자', '위와 같음'],
    ['생산입고 III-소모품목 선택|담당자', '위와 같음'],
    ['생산입고_소모현황 I|담당자', '위와 같음 — 거를 값이 자료에 없다'],
    ['작업지시서작업처리|담당자', '작업지시의 담당자로 거르는 칸을 아직 안 만들었다'],
    ['작업지시서효율현황|담당자', '위와 같음'],
    ['작업지시서효율현황|적요', '적요로 거르는 칸을 아직 안 만들었다'],
    ['휴가잔여일수현황|적요', '위와 같음'],
