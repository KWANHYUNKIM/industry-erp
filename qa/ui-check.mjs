 ...
 */
{
  ...
  const NO_COLUMN = new Map([
     ...
     */
    ['발주서입력|추가문자형식1', '원본은 격자 열을 <b>회사가 직접 늘린다</b> — 추가문자·추가숫자·추가일자·추가코드 형식으로 스물다섯 칸을 열어 둔다. 우리 추가항목은 판매·구매 전표(머리·줄)까지고 <b>발주서 화면이 아직 안 읽는다</b>'],
    ['발주서입력|추가문자형식2', '위와 같음'],
    ['발주서입력|추가문자형식3', '위와 같음'],
    ['발주서입력|추가문자형식4', '위와 같음'],
    ['발주서입력|추가문자형식5', '위와 같음'],
 ...
 */
{
  ...
  const NO_FIELD_ON = new Map([
    ...
    ['출하지시서조회|최초작성일자', '이 화면은 조건 줄이 이미 아홉이라 만든 때·고친 때는 출하조회에서 본다'],
    ['출하지시서조회|최종작업일자', '위와 같음(출하조회의 [최종수정일시]가 같은 값이다)'],
    ...
    ['구매조회|최초작성일자', '만든 때가 응답에 없다 — BaseTimeEntity 는 들지만 PurchaseResponse 가 안 싣는다'],
    ['구매조회|최종작업일자', '고친 때는 실었으나 원본의 이 칸은 <b>작업일자</b>다 — 무엇을 가리키는지 못 재어 지어내지 않는다'],
    ...
    ['판매조회|최초작성일자', '만든 때가 응답에 없다 — BaseTimeEntity 는 들지만 SalesResponse 가 안 싣는다'],
     ...
     */
    ['단가요청현황|참조', '발주 전표에 다른 문서를 가리키는 칸이 없다'],
    ['발주계획현황|참조', '위와 같음'],
