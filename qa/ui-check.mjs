 ...
 */
{
  ...
  const NO_COLUMN = new Map([
     ...
     */
    ['품질검사요청조회|품목명(요약)', '요청 한 건이 품목 하나짜리라 요약할 것이 없다 - [품목명] 이 곧 그 값이다'],
    ['품질검사요청조회|수량합계', '위와 같음 - 품목이 하나라 합칠 줄이 없다. [수량] 이 곧 그 값이다'],
    ['품질검사요청조회|연결전표', '검사요청이 근거 전표를 물지 않는다 - 품목과 로트만 들고 시작한다'],
    ['품질검사요청조회|생성한전표', '요청에서 검사 전표를 자동으로 만들지 않는다 - 상태만 요청→검사완료로 옮긴다'],
 ...
 */
{
  ...
  const NO_FIELD_ON = new Map([
    ...
    ['견적서조회|적요2', '위와 같음'],
    ['견적서조회|적요3', '위와 같음'],
    ['견적서조회|문자형식1', '추가항목은 칸 이름을 박지 않고 사용자정의필드에서 이름을 지어 정의한다'],
    ['견적서조회|문자형식2', '위와 같음'],
    ['견적서조회|문자형식3', '위와 같음'],
    ['견적서조회|문자형식4', '위와 같음'],
    ['견적서조회|문자형식5', '위와 같음'],
