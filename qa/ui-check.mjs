 ...
 */
{
  ...
  const NO_FIELD_ON = new Map([
    ...
    /*
     * <b>출하조회 아홉</b> — 사본에 열뿐이던 것을 실측 스물넷으로 갈아 끼우며 드러났다.
     * 이 화면의 조건 판에는 <b>[보내기]에 딸린 것</b>이 여럿이다(연락처·주소·제목,
     * 그리고 문자형식·장문형식). 우리는 전표를 내보내는 기능이 통째로 없어 그 줄들이 없다.
     * (문자형식·장문형식은 조건인지 [보내기] 서식 목록인지 못 갈라 사본에 아예 안 적었다.)
     */
    ['출하조회|오더관리번호', '출하 전표에 오더 참조가 없다 — 오더관리는 수주에 붙는다'],
    ['출하조회|거래처관리담당자', '출하 응답에 거래처 담당자가 없다 — 거래처 마스터를 따로 받아 이어야 한다'],
    ['출하조회|연락처', '[보내기]에 딸린 칸이다 — 전표를 내보내는 기능이 없다'],
    ['출하조회|주소', '위와 같음 — [보내기]에 딸린 칸이다'],
    ['출하조회|제목', '위와 같음 — 보낼 때 붙이는 제목이다'],
    ['출하조회|최초작성일자', '만든 때가 응답에 없다 — BaseTimeEntity 는 들지만 출하 응답이 안 싣는다'],
    ['출하조회|최종수정일시', '고친 때가 응답에 없다 — 판매·구매·견적에는 실었으나 출하는 아직이다'],
