 ...
 */
{
  ...
  for (const k of ['발주서입력|저장(F8)', '매출계획입력|저장(F8)']) NO_BUTTON.delete(k)
 ...
 */
{
  ...
  const NO_FIELD = new Map([
    ...
    ['양식구분', '위와 같음'],
    ['결재방표시', '인쇄물에 <b>결재란</b>을 찍는 서식 설정이라 조회 조건이 아니다 — 원본은 조회 화면마다 이것을 둔다. 우리는 인쇄 판을 아직 안 건드렸다'],
 ...
 */
{
  ...
  const NO_OPTION = new Map([
     ...
     */
    ['거래처관리대장 I|거래처관계기준', '아직 원본을 안 열어 봤다 — 채권현황처럼 라디오일 것이나 재고 나서 고친다'],
    ['거래처관리대장 I|개별거래처기준', '위와 같음'],
    ['거래처관리대장 II|거래처관계기준', '거래처등록 화면이라 합칠 잔액 자체가 없다'],
    ['거래처관리대장 II|개별거래처기준', '위와 같음'],
    ['거래처별채권|거래처관계기준', '아직 원본을 안 열어 봤다 — 채권현황처럼 라디오일 것이나 재고 나서 고친다'],
    ['거래처별채권|개별거래처기준', '위와 같음'],
    ['거래처별채무|거래처관계기준', '위와 같음'],
    ['거래처별채무|개별거래처기준', '위와 같음'],
