 ...
 */
{
   ...
   *
   * <p>현황 화면은 그럴 필요가 없다 — 거래처별채권을 다시 열어 보니 조건 판이
  ...
  const NO_FIELD_ON = new Map([
     ...
     */
    /*
     * <b>여기 적혀 있던 근거가 거짓이었다.</b> '판매 전표에 오더 참조가 없다' 고 했으나
     * <code>SalesLine.sourceOrder</code> 가 근거 수주를 물고 있고, 응답도
     * <code>sourceDocNo</code> 로 싣고, <b>목록의 [근거전표] 열에 이미 찍고 있다</b>.
     * 구매·출하도 같다(<code>PurchaseLine.sourceOrder</code>·<code>Shipment.salesOrder</code>).
     *
     * <p>그래도 아직 안 만든다. <b>원본의 [오더관리번호]가 무엇인지 못 쟀기 때문</b>이다 —
     * 그 수주 <b>전표번호</b>인지, 수주에 따로 매기는 <b>관리번호</b>인지 둘 중 하나인데
     * 우리에게 뒤엣것은 없다(수주가 드는 것은 orderNo 와 오더관리<b>유형</b>뿐이다).
     * 이름이 같다고 짐작해서 만들면 고른 값과 걸리는 값이 어긋난다 — 회계반영 화면의
     * [거래처관리담당자]가 그렇게 틀렸었다. 원본을 열어 재고 나서 만든다(보드에 적어 두었다).
     */
    ['판매조회|오더관리번호', '근거 수주는 물고 있고 [근거전표] 열에 이미 찍는다 — 다만 원본 [오더관리번호]가 그 전표번호인지 수주에 따로 매기는 관리번호인지 아직 못 쟀다. 지어내지 않는다'],
    /*
     ...
     */
    ['출하조회|오더관리번호', '위와 같음 — Shipment.salesOrder 를 물고 [수주번호] 열에 이미 찍는다. 원본 [오더관리번호]의 뜻을 아직 못 쟀다'],
    ...
    ['구매조회|오더관리번호', '위와 같음 — PurchaseLine.sourceOrder 를 물고 [근거전표] 열에 이미 찍는다. 원본 [오더관리번호]의 뜻을 아직 못 쟀다'],
