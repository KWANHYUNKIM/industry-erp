 ...
 *
 * <p><b>화면이 가진 자리가 곧 그 화면의 자료는 아니다.</b> 품목등록은 품목 말고도
 * 거래처(구매처 코드도움)·품목그룹·관리항목을 함께 부른다. 그래서 [적요]를 찾다가
 * <code>PartnerResponse</code> 의 remark 를 집어 "서버는 이미 보내고 있다" 고 귀띔했는데,
 * 품목에는 remark 가 <b>아예 없다.</b> 아홉 중 셋을 그렇게 헛짚었고, 좁힌 뒤에도 하나 남았다.
 *
 * <p>그 화면이 <b>고쳐 쓰는</b> 자리(post·put·delete 를 보내는 곳)가 그 화면의 자료다.
 * 부르기만 하는 자리는 남의 것이라 빼고 본다. 고쳐 쓰는 데가 없는 조회 화면은 그대로 둔다.
 */
const responseSrcFor = (pageSrc) => {
  const 고쳐쓰는곳 = new Set()
  for (const m of pageSrc.matchAll(/api\.(?:post|put|delete|patch)(?:<[^>]*>)?\([`']([^`'$]+)/g)) {
    const seg = m[1].split('?')[0].replace(/\/$/, '').split('/').filter(Boolean)[0]
    if (seg) 고쳐쓰는곳.add('/' + seg)
  }
  const out = []
  for (const m of pageSrc.matchAll(/api\.get<[^>]*>\('([^']+)'/g)) {
    const path = m[1].split('?')[0]
    if (고쳐쓰는곳.size && !고쳐쓰는곳.has('/' + path.split('/').filter(Boolean)[0])) continue
    const rec = RESPONSE_FIELDS.byPath.get(path)
 ...
 */
{
  ...
  const NOT_MASTER = new Map([
     ...
     */
    ['inventory/ItemsPage.tsx|품목공유여부', '품목을 고르는 칸이 아니라 <b>공유/공유안함</b> 두 값이다'],
    ['inventory/ItemsPage.tsx|생산전표생성-창고이동', '창고를 고르는 칸이 아니라 <b>생성함/생성안함</b> 두 값이다'],
    ['trade/PartnersPage.tsx|외화거래처', '거래처를 고르는 칸이 아니라 <b>원화/외화</b> 두 값이다(사본의 id 가 ddlSforeignFlag — 깃발이다)'],
  ])
  const loose = []
 ...
 */
{
  ...
  const FILE_OF = new Map([
    ...
    ['Invoice / Packing List', 'trade/ExportPage.tsx'],
    ...
    ['A/S접수현황', 'quality/AsStatusPage.tsx'],
    ['품질검사현황', 'quality/QualityStatusPage.tsx'],
    ...
    ['재고현황', 'inventory/CurrentStockPage.tsx'],
 ...
 */
{
   ...
   *
   * <p>목록을 훑어 <b>어느 것이 이름만 다른 것인지</b> 가려 두었다. 다음 사람이 같은 것을
   * 다시 파헤치지 않도록 적는다.
   * <ul>
   *   <li><b>매출계획·매출계획조회 [금액]</b> — 있다. 우리는 같은 표에 계획과 실적을 나란히
   *       보여 주느라 [계획금액]·[실적금액] 으로 갈라 적었다. [금액] 하나로 부르면 어느 쪽인지
   *       알 수 없다. 원본 그 화면의 열은 일곱이고(일자-No.·거래처명·담당자명·창고명·
   *       프로젝트명·품목명·금액) 실적을 함께 두지 않는다.</li>
   *   <li><b>매출계획·매출계획조회 [일자-No.]</b> — <s>진짜로 없다</s> <b>만들었다.</b>
   *       계획 한 줄을 가리킬 이름이 없어 "어느 계획을 지웠다" 고 말할 방법이 없었다.
   *       이제 SP-yyyyMMdd-NNNN 으로 채번한다(V205/V78).</li>
 ...
 */
{
  ...
  const NO_COLUMN = new Map([
     ...
     */
    ['A/S접수현황|수량', 'A/S 접수에 수량 칸이 없다 - 부품 수량은 고치며 쓴 것이지 받은 대수가 아니다'],
    /*
    ...
    /*
     * <b>재고수불부(E040702) [거래처명]</b> - 2026-09-09 원본 격자 실측.
     * 재고 움직임 한 줄에 <b>누구와 거래한 것인지</b>가 없다 - <code>StockTransaction</code>
     * 은 품목·창고·수량·단가만 든다. 거래처를 달려면 <code>inventory</code> 가
     * <code>trade</code> 를 알아야 하는데 <b>그 방향은 순환</b>이다(CLAUDE.md 4.1:
     * trade -> inventory). id 만 두고 화면이 이름을 붙이는 길은 있지만,
     * <b>지금은 아무도 그 id 를 쓰지 않는다</b> - 없는 값을 열로 만들지 않는다.
     */
    ['재고수불부|거래처명', '재고 움직임 줄에 거래처가 없다 - inventory 가 trade 를 참조하면 순환이라 id 조차 안 둔다'],
    /*
     ...
     */
    ['A/S소모현황|소모(판매)번호', '우리 A/S 는 부품을 재고에서 빼기만 하고 판매 전표를 만들지 않는다'],
    ['A/S소모현황|부가세', '소모부품 줄에 부가세 칸이 없다 - 0 을 찍으면 면세로 읽힌다'],
    /*
     * <b>작업내역조회(E040431) [일자-No.]</b> - 2026-09-09 원본 격자 실측.
     * 원본은 작업내역 <b>전표에 번호를 매기고</b> 일자와 한 칸에 적는다.
     * 우리 <code>WorkResult</code> 는 <b>번호가 없다</b> - 날짜만 든다. 없는 번호를
     * 붙여 적을 수 없어 [일자] 로만 찍는다. 번호를 매기려면 컬럼과 마이그레이션이
     * 본사·테넌트 양쪽에 필요하다(매출계획이 그렇게 SP-yyyyMMdd-NNNN 을 얻었다).
     */
    ['작업내역조회|일자-No.', '작업내역 전표에 번호가 없다 - 날짜만 든다(번호를 매기려면 컬럼과 마이그레이션이 필요하다)'],
    /*
     * <b>불량률파악보고서(E040512) [생산수량]·[불량수량]</b> - 2026-09-09 원본 격자 실측.
     * 원본은 <b>생산한 수</b>를 밑에 깔고 불량률을 낸다. 우리 숫자는 <b>검사</b>에서
     * 나온다(<code>QualityInspection.inspectedQty</code>·<code>defectQty</code>) -
     * 검사를 안 한 생산분은 애초에 줄에 없다. 같은 칸에 원본 이름을 붙이면
     * <b>생산 전부를 센 것처럼</b> 읽혀 불량률이 실제보다 높아 보인다.
     * 우리 이름([검사수량]·[검사불량])이 그 값을 정확히 가리킨다.
     */
    ['불량률파악보고서|생산수량', '우리 숫자는 검사에서 나온다 - 검사 안 한 생산분은 줄에 없어 생산수량이라 부르면 거짓이 된다'],
    /*
     * <b>실제원가현황(E040804) [생산공정명]</b> - 2026-09-09 원본 격자 실측.
     * 원본은 재공을 <b>공정별로</b> 들고 있어 원가집계표의 넷째 칸이 생산공정명이다.
     * 우리 재고는 <b>창고 단위</b>라 공정별 재공이 없다 - 그 칸에 넣을 값이 아예 없다
     * (화면 머리말에 진작 적어 둔 이유다). 창고를 공정처럼 쓰면 이름만 같고 다른 값이 된다.
     */
    ['실제원가현황|생산공정명', '우리 재고는 창고 단위라 공정별 재공이 없다 - 그 칸에 넣을 값이 없다'],
    ['차이분석|생산공정명', '위와 같음 - 우리 재고는 창고 단위라 공정별 재공이 없다'],
    ['표준원가현황|생산공정명', '위와 같음 - 우리 재고는 창고 단위라 공정별 재공이 없다'],
    ['불량률파악보고서|불량수량', '위와 같음 - 우리 것은 검사에서 나온 [검사불량]이다'],
    ['단가요청현황|단가(수취단가)', '위와 같음'],
    ['단가요청현황|단가(확정단가)', '위와 같음'],
    /*
    ...
    /*
     * <b>창고이동현황(E040505) [금액(수량*입고단가)]</b> - 2026-09-09 원본 격자 실측.
     * 자가사용현황·재고조정현황과 같은 까닭이다 - 사내에서 재고만 옮기는 전표라
     * 단가를 안 매긴다. 재고 단가로 되짚어 채우면 원본이 말하는 금액과 다른 값이 된다.
     */
    ['창고이동현황|금액(수량*입고단가)', '창고이동에 단가를 안 매긴다 - 곱할 입고단가를 이 전표에 안 남긴다'],
    /*
     ...
     * 재고조정의 [금액]은 자가사용과 같은 까닭이다 - 단가를 안 매긴다.
     */
    ['대체사용현황|불량수량', '기타이동 전표는 증감 한 수만 든다 - 불량분과 정상분을 갈라 적지 않는다'],
    ['대체사용현황|정상수량', '위와 같음'],
    ['재고조정현황|금액(수량*입고단가)', '기타이동에 단가를 안 매긴다 - 곱할 입고단가를 이 전표에 안 남긴다'],
    /*
     ...
     * 전표라 상대가 없고 단가도 안 매긴다 - 두 칸 다 채울 값이 없다.
     * (원본은 '수량*입고단가' 라고 열 이름에 계산식까지 적어 두는데, 우리는
     * 그 입고단가를 이 전표에 남기지 않는다.)
     */
    ['자가사용현황|거래처명', '기타이동은 사내에서 재고만 움직이는 전표라 상대가 없다'],
    ['자가사용현황|금액(수량*입고단가)', '기타이동에 단가를 안 매긴다 - 곱할 입고단가를 이 전표에 안 남긴다'],
    /*
     * <b>생산불출현황(E040409) [생산금액]</b> - 2026-09-09 원본 격자 실측.
     * 원본은 낸 자재에 <b>금액</b>을 매겨 같이 찍는다. 우리 불출(MaterialIssue)은
     * <b>수량만</b> 든다 - 단가를 안 매기므로 곱할 값이 없다. 재고 단가로 되짚어
     * 채우면 <b>원본이 말하는 금액과 다른 값</b>에 같은 이름표를 붙이게 된다.
     */
    ['생산불출현황|생산금액', '불출에 단가를 안 매긴다 - 수량만 든다'],
    /*
     ...
     */
    ['품질검사현황|검사방법', '샘플링·전수를 갈라 적지 않는다 - 검사수량만 든다'],
    ['품질검사현황|시료', '위와 같음 - 뽑은 시료 수를 따로 안 든다'],
    /*
     ...
     */
    /*
     * 예전 이유: "우리 Self-Customizing 은 화면 칸까지고 <b>전표 격자 열로는 안 뻗는다</b>".
     * 두 겹으로 틀렸다. 격자 열로 뻗는다 — 판매·구매 입력이 SALES_LINE·PURCHASE_LINE 을
     * 읽어 줄마다 추가항목을 그린다. 진짜 한계는 다른 데 있었다: 정의 화면이 만들 수 있는
    ...
    ['매출계획입력|품목코드', '위와 같음'],
    /*
     * 아래 여섯은 <b>바로 윗줄이 아닌 것</b>을 가리키고 있었다 — '위와 같음' 이라 적혀
     ...
     * 없다' 였는데, 부품 격자는 <b>있었다</b>(부품·창고·수량·단가·금액). AsPart.remark 도
     * AsPartResponse.remark 도 진작 있었고 그 격자에만 칸이 없었다.
     */
    ['A/S접수입력|추가수량', '위와 같음'],
    ['A/S접수입력|단위', '위와 같음'],
    ['A/S접수입력|관리항목', '위와 같음'],
     ...
     */
    ['생산불출조회|불러온 전표No.',
      '우리는 작업지시서에서만 불러온다 — [작업지시서] 열과 같은 값이다.'
      + ' 원본은 [전표불러오기] 로 여러 전표에서 끌어와 그 번호가 줄마다 다르다'],
     ...
     */
    /*
     * 여기에 같은 키가 <b>두 번</b> 적혀 있었다. Map 은 뒤엣것만 남으므로 앞의 이유는
     * 아무도 안 읽는 글이 되어 있었다 — 그리고 뒤엣것('외화 전표를 만들지 않는다')은
     * <b>사실이 아니다.</b> 수출오더는 통화를 반드시 들고(currency_id NOT NULL),
     * 고시환율 마스터(ExchangeRate)도 있다. 없는 것은 <b>매입·매출 전표의 통화 칸</b>이다.
     */
    ['구매단가일괄변경|환율',
      '매입 전표에 통화 칸이 없다(수출오더에는 있다) — 환산까지 가야 하는 일이라 열 하나로 끝나지 않는다'],
    ...
    ['원가생성/수정|기타', '위와 같음'],
    /*
     * 다섯 줄 다 '그 팝업을 안 만든다' 라고 적혀 있었는데 <b>사실이 아니다.</b>
     * [생성일자]·[생산계획기간]·[기준품목]·[적요]는 우리 화면에 <b>조건으로 있고</b>,
     ...
     */
    ['원가생성_수정|원가계산방법', '표가 아니라 원가생성 실행 화면의 칸이다'],
    ['원가생성_수정|사전작업', '위와 같음'],
    ['원가생성_수정|원가계산', '위와 같음'],
    ['원가생성_수정|원가현황', '위와 같음'],
    ['원가생성_수정|기타', '위와 같음'],
     ...
     * 그 칸들이 격자 열처럼 잡혔다. 위의 같은 이름들과 <b>한 글자도 다르지 않은 자리</b>라
...
const SHELL_BUTTONS = new Set(['사이트맵', 'Option', '도움말', 'Search(F3)', '찾기(F3)', '다시 작성',
  ...
  // 원본은 같은 단추를 <b>두 가지로 적는다</b> — [다시 작성]·[다시작성]. 띄어쓰기만 다르다.
  '다시작성',
  /*
   * 기간 묶음은 화면마다 다르다 — A/S 조회에는 [직전분기]·[직전반기]가 붙는다(AS_PICKS).
   * <b>EcPeriodPicks 가 상수 배열로 그려서</b> 화면 파일에는 그 글자가 없다.
 ...
 */
{
  ...
  const NO_BUTTON = new Map([
     ...
     */
    ['거래처리스트|H',
      '원본 [H]는 그 화면의 변경이력을 여는 버튼이다(사본 button id=history) —'
      + ' <b>거래처 마스터</b>의 변경이력은 남기지 않는다.'
      + ' (전표 쪽은 다르다 — 발주서는 단계마다 이력을 남기고 화면에서 펼쳐 본다)'],
    ['단가적용순서설정|H', '위와 같음'],
  ...
  /*
   * <b>[검색(F8)]·[저장/검색(F8)] — 조건을 바꾸면 바로 반영된다.</b>
   *
   * <p>원본은 조건 판에 실행 단추를 두고 <b>눌러야</b> 결과가 바뀐다. 우리는 조건을 만지면
   * 그 자리에서 목록이 바뀐다 — 글자 조건은 받아 둔 줄을 브라우저에서 거르고, 기간이
   * 바뀔 때만 서버에 다시 묻는다(<code>useEffect([from, to])</code>). 아래 31화면 전부
   * 그렇게 되어 있는지 <b>하나씩 열어 확인했다</b> — 글자를 칠 때마다 서버를 부르는 곳은
   * 하나도 없다.
   *
   * <p>거래처관리대장에 진작 같은 이유를 적어 두었는데, 74화면을 한꺼번에 걸면서
   * 39곳이 '없는 단추'로 세어졌다. 안 만든 것이 아니라 <b>안 두기로 한</b> 것이다.
   * (조건 판에 실행 단추를 둔 화면도 68곳 있다 — 서버를 다시 부르는 것이 무거운 자리다.)
   */
  for (const screen of [
    'A/S소모현황', 'A/S접수', 'A/S접수조회', 'A/S접수현황', '거래이력조회', '거래처등록', '거래처특별단가그룹등록', '견적서', '견적서조회',
  ...
  /*
   * <b>[저장/검색(F8)] — 조건을 기억해 두는 자리를 만들지 않았다.</b>
   *
   * <p>원본의 이 단추는 <b>지금 조건을 그 화면의 기본값으로 저장하고</b> 조회한다.
   * 아래 여섯 화면에는 <b>[검색(F8)] 은 있다</b> — 없는 것은 '저장' 쪽이다.
   * 우리는 화면을 열 때마다 실측한 기본 기간으로 시작하고, 조건을 사람별로 기억하지 않는다.
   */
  for (const screen of ['재고수불부', '경영자보고서', '일보', '창고별재고현황',
    '재고변동표', '일별재고현황', '조건별검색']) {
    NO_BUTTON.set(screen + '|저장/검색(F8)',
      '조건을 그 화면의 기본값으로 <b>저장</b>하는 자리를 만들지 않았다 — [검색(F8)] 은 있다')
  }

  /*
   * <b>[Email]·[보내기] — 전표를 밖으로 보내지 않는다.</b>
   *
   * <p>원본은 전표 화면에서 그 문서를 <b>거래처 메일로</b> 보내거나([Email]),
   * <b>상대 회사의 이카운트로</b> 넘긴다([보내기]). 둘 다 바깥으로 나가는 일이다.
   *
   * <p>우리 메일은 <b>사내메일</b>이다 — <code>POST /api/mails</code> 는 우리 사용자끼리
   * 주고받고, 외부 메일 서버 연동은 자리만 있고 붙이지 않았다(MailController 의
   * '외부 메일 서버 연동 자리'). 거래처 주소로 문서를 내보내는 길이 없다.
   * 거래명세서인쇄·설문조사조회 등에 진작 같은 이유를 적어 두었는데,
   * 74화면을 한꺼번에 걸면서 21곳이 '없는 단추'로 세어졌다.
   */
  for (const screen of [
    'A/S접수', 'A/S접수조회', 'A/S접수현황', '견적서조회', '매출계획', '매출계획조회', '매출계획현황', '발주서', '발주서조회',
    '발주서현황', '생산불출', '자가사용현황', '창고이동조회', '품질검사현황']) {
    for (const b of ['Email', '보내기']) {
      NO_BUTTON.set(screen + '|' + b, '전표를 밖으로 보내지 않는다 — 우리 메일은 사내메일이다')
    }
  }

  /*
   * <b>[진행상태변경] — 그 전표에 단계가 없다.</b>
   *
   * <p>창고이동(StockTransfer)과 생산불출(MaterialIssue)에는 <b>상태 칸 자체가 없다</b> —
   * 넣는 순간 재고가 움직이고 끝이라 거쳐 갈 단계가 없다. A/S접수는 단계가 있지만
   * 다음 단계가 [수리완료]면 <b>수리내역을 받아야</b> 해서, 고른 열 줄에 열 번 묻는 꼴이 된다.
   *
   * <p>단계가 있고 값을 안 물어도 되는 자리는 만들었다 — 발주서(단가확정 끝난 줄을
   * 한 번에 발주확정)·견적서(안 보낸 줄을 한 번에 발송)·출하지시서조회.
   */
  ...
  /*
   * <b>[바코드(품목)] — 바코드를 찍지 않는다.</b> 품목에 바코드 칸은 있지만 라벨을 뽑는
   * 자리를 만들지 않았다. 작업내역조회·소요시간계산에 진작 같은 이유를 적어 두었다.
   */
  for (const screen of ['발주서', '발주서조회', '창고이동조회', '생산불출',
    '견적서조회', 'A/S접수', 'A/S접수조회']) {
    NO_BUTTON.set(screen + '|바코드(품목)', '바코드를 찍지 않는다 — 품목에 칸은 있지만 라벨을 뽑지 않는다')
  }

  /*
   * <b>[신규(F2)] — 그 화면에서 새로 만들 것이 없다.</b> 셸은 <code>onNew</code> 를 준
   * 화면에만 이 단추를 그린다(EcListShell). 아래 넷은 <b>만드는 자리가 다른 화면</b>이거나
   * 계산해서 나오는 것이라 손으로 새로 넣지 않는다.
   *
   * <p>발주서·견적서는 표 위에 따로 떠 있던 것을 <b>셸의 그 자리로 옮겼다</b> —
   * 원본처럼 아래 단추줄 맨 앞이다.
   */
  for (const [screen, why] of [
  ...
  /*
   * <b>[다른전표생성] — 만들 수 있는 전표가 하나뿐이라 그 이름으로 바로 부른다.</b>
   *
   * <p>원본은 이 단추를 눌러 <b>무슨 전표를 만들지 고르게</b> 한다. 우리 발주서에서 나오는
   * 것은 구매전표 하나뿐이고 견적서에서 나오는 것은 수주 하나뿐이라, 고를 것이 없다 —
   * 줄마다 [입고전환]·[수주전환] 이라고 <b>무엇이 되는지</b>를 적어 두었다.
   */
  for (const [screen, why] of [
    ['발주서', '나오는 전표가 구매 하나뿐이다 — 줄마다 [입고전환]이라고 이름 붙여 두었다'],
    ['발주서조회', '위와 같음'],
    ['견적서조회', '나오는 전표가 수주 하나뿐이다 — 줄마다 [수주전환]이다'],
    ['A/S접수', 'A/S접수에서 다른 전표를 만들지 않는다 — 소모부품은 그 안에서 적는다'],
    ['A/S접수조회', '위와 같음'],
  ]) NO_BUTTON.set(screen + '|다른전표생성', why)

  /*
   * <b>[전자결재] — 결재에 붙일 수 있는 전표가 셋뿐이다.</b>
   *
   * <p><code>ApprovalDocumentVoucher</code> 가 무는 것은 <b>판매·구매·비용</b> 뿐이다.
   * 발주서·견적서·창고이동·생산불출은 붙는 자리가 아예 없다.
   * 생산입고조회에 진작 같은 이유를 적어 두었다('생산 전표를 결재에 올리지 않는다').
   */
  for (const screen of ['발주서', '발주서조회', '창고이동조회', '생산불출', '견적서조회']) {
    NO_BUTTON.set(screen + '|전자결재',
      '결재에 붙는 전표는 판매·구매·비용 셋뿐이다(ApprovalDocumentVoucher) — 이 전표는 붙는 자리가 없다')
  }

  /*
   * <b>[선택삭제] — 지우는 자리가 없거나 지울 수 있는 줄이 아니다.</b>
   * 지울 자리가 서버에 있는 화면에는 만들었다(발주서·견적서·매출계획·프로젝트계획·
  ...
  /*
   * <b>원본 입력 격자의 공통 툴바.</b> 원본은 전표를 격자로 넣으면서 위쪽 툴바에
   * 정렬·My품목·전표불러오기·재고불러오기·바코드·검증·저장(F8)·저장/전표(F7)·리스트를 둔다.
   * 우리 입력 폼에는 <b>[등록] 하나뿐</b>이다 — 생산 격자 화면들에 진작 같은 이유를 적어 두었고,
   * 74화면을 한꺼번에 걸면서 네 화면이 더 걸렸다.
   */
  for (const screen of ['발주서입력', '품질검사요청입력', 'A/S접수입력', '매출계획입력']) {
    for (const b of ['정렬', 'My품목', '주문', '판매', '구매', '생산', '이동', '발주', '계획',
      'A/S접수', '소요', '단가요청', '연결전표', '전표불러오기', '재고불러오기', '품목별안전재고',
      '생성한 전표', '바코드', '전표 바코드', '검증', '재고', '수량±', 'F4',
      '거래내역보기(발주)', '거래내역보기(A/S접수)',
      '저장(F8)', '저장/전표(F7)', '리스트']) {
      NO_BUTTON.set(screen + '|' + b, '원본 격자 화면의 공통 툴바 — 우리 폼에는 [등록] 하나뿐이다')
    }
  }
  /* 묶어서 뺐지만 실제로는 있는 것들 — 아래 [낡은 예외] 단언이 이 목록을 강제한다. */
  /*
   * <b>[My품목] 은 격자가 있는 두 화면에서 이제 만들었다.</b> 묶음 이유는 "우리 폼에는
   * [등록] 하나뿐이다" 였는데 발주서입력·생산불출입력은 실제로는 <b>줄을 여럿 넣는 격자</b>다
  ...
  for (const [k, why] of [
    ['거래처등록|SMS', '문자 발송을 붙이지 않았다'],
    ['단계별재고조정|저장(F8)', '입력 폼의 단추는 [등록] 하나다'],
    ['생산계획/MRP생성|6개월(+1개월)', '이 화면의 기간 묶음에 반년짜리를 두지 않았다'],
    ['A/S접수|접수증인쇄', '접수증 서식을 따로 만들지 않았다 — 목록은 [인쇄]로 낸다'],
    ['A/S접수조회|접수증인쇄', '위와 같음'],
    ['품목등록|관계설정', '품목 사이 관계(대체품·세트) 개념이 없다'],
  ]) NO_BUTTON.set(k, why)
  ...
  /*
   * 생산불출조회는 입력과 <b>같은 파일</b>(IssuePage)이라, 입력 격자에 [My품목]을 달자
   * 조회 쪽에서도 보인다. <b>이 자리에서 지워야 한다</b> — 위쪽 삭제 목록에 적으면
   * 바로 앞 묶음이 다시 채워 넣어 아무 일도 일어나지 않는다.
   */
  /*
   * [회계반영]은 <b>편집이 아니라 넘기는 일</b>이라 판매·구매조회 묶음 이유
   * ('조회에서 바로 고치지만 우리는 입력 화면을 따로 둔다')가 안 맞았다.
   * 서버에 진작 자리가 있었고(POST /accounting-reflection/reflect), 이 화면은 고른 줄과
   * 일괄 동작이 이미 배선돼 있어 잇기만 하면 됐다.
   * <b>이 자리에서 지운다</b> — 위쪽 삭제 목록에 적으면 판매·구매조회 묶음이 다시 채워 넣는다.
   */
  NO_BUTTON.delete('판매조회|회계반영')
   ...
   */
  for (const s2 of ['생산입고II-소모품목 선택', '생산입고 III-소모품목 선택']) {
    NO_BUTTON.set(s2 + '|재고불러오기',
      '이 단추가 채우는 칸의 이름을 모른다 — 쓰기 화면이라 원본을 열어 재지 않는다(발주서입력은 열 너비 대조표에 있어 만들 수 있었다)')
  }
  /*
 ...
 */
{
  ...
  /*
   * ⚠ <b>전표 조회 화면의 사본은 조건이 덜 적혀 있다.</b>
   *
   * <p>원본 조회 화면의 조건 판에는 <b>접힌 줄(▾)</b>이 있고, 펼치면 조건이 곱절로 나온다.
  ...
  const NO_FIELD = new Map([
    ...
    ['양식구분', '위와 같음'],
    /*
     * <b>[기타] — 우리도 묶는다. 아직 못 묶은 것은 '안 잰 것' 이지 '안 만든 것' 이 아니다.</b>
     *
     * <p>예전 이유는 '우리는 조건 판에 그대로 편다' 였다. <b>지금 코드와 어긋난다</b> —
     * 예순다섯 화면 가운데 <b>열여섯</b>이 이미 [기타]로 묶고 있다(재고 출력물 계열이
     * 원본을 실측하면서 그렇게 됐다). 2026-09-07 에 원본을 다시 열어 확인했다:
     * 거래처별채권 조건 판에 <code>기타  ☑ 사용중단거래처포함</code> 처럼 한 줄로 묶인다.
     * <b>묶는 것이 원본이고, 우리 방식도 그쪽이다.</b>
     *
     * <p>그런데 남은 마흔아홉 화면은 <b>바로 만들 수가 없다.</b> [기타] 라는 줄이 있다는
     * 것만 알고 <b>그 안에 무엇이 들어가는지를 안 쟀다</b>. 사본 조건 대조표에는 '기타'
     * 한 낱말로만 적혀 있고, 체크박스 대조표는 <b>그 화면의 체크박스 전부</b>를 적은 것이라
     * 어느 것이 [기타] 소속인지 가려 주지 않는다(구매일괄회계반영의 여덟에는 원재료·부재료
     * 같은 품목구분이 섞여 있다).
     *
     * <p>빈 [기타] 껍데기를 만들면 검사만 통과하고 화면은 거짓말이 된다. 그러니 남은 것은
     * <b>원본을 화면별로 열어 [기타] 속을 재는 일</b>이지 코드를 미는 일이 아니다.
     * 재고 출력물 다섯을 그렇게 고쳤고(fc154e6 계열), 나머지도 같은 길이다.
     */
    ['기타', '원본은 자잘한 체크박스를 이 이름으로 묶고 <b>우리도 묶는다</b>(65화면 중 19 완료). 남은 화면은 [기타] 안에 무엇이 들어가는지를 아직 안 쟀다 — 지어내지 않는다'],
    ['내.외자구분', '국내/수입 구분을 전표에 두지 않는다'],
    /*
     * <b>[관리항목]은 이 목록에서 뺐다 — 열다섯 화면 전부 만들었다.</b>
     *
     * <p>적혀 있던 이유는 이랬다: "관리항목은 품목 마스터에 붙는 값이라 전표에는 없다".
     * 앞말은 사실이다 — trade·accounting 어느 응답 DTO 에도 없다. 그런데 <b>그래서 못
     * 만든다</b>는 틀렸다. 판매현황이 이미 다른 길로 만들어 두고 있었다: 품목 마스터를
     * 통째로 받아 <b>줄의 itemId 로 화면에서 잇는다</b>. 전표에 그 값이 없어도 품목을
     * 알면 관리항목을 안다. 그 방법을 utils/itemMgmtItems 로 나눠 열셋에 더 붙였다.
     *
     * <p>묶음 이유 하나가 <b>이미 만든 것까지</b> 덮고 있었다는 뜻이기도 하다 —
     * 판매현황·품목등록은 진작 되어 있었는데 이 한 줄에 가려 아무도 안 봤다.
     */
    /*
     * <b>[데이터 보기형식]은 이 목록에서 뺐다 — 서른네 화면을 다 만들었다.</b>
     *
     * <p>예전에 여기 적혀 있던 것: "이름만 보고 우리 [내역/집계] 알약에 붙일 뻔했다.
     * 원본의 이 조건은 상세/요약이 아니라 <b>[그래프로 보기]</b> 체크 하나다." 그 관찰은
     * 옳았고 지금도 유효하다 — 다만 뒤에 붙은 "그 <b>여덟</b> 화면에 아직 그래프가 없다"가
     * 낡아 있었다. 걸린 화면은 여덟이 아니라 <b>서른넷</b>이었고, 그중 열셋은 그때 이미
     * 그래프가 있었다. 숫자가 굳으면 남은 일이 실제보다 작아 보인다.
     *
     * <p>화면별 예외도 하나도 안 남겼다. 위임하는 넷(할인현황 셋·지급현황)은 검사가
     * 못 볼 줄 알고 예외를 적었다가 <b>틀린 것을 확인했다</b> — 검사는 위임 너머를 본다.
     * 짐작으로 예외를 적으면 <b>이미 되어 있는 것을 못 되어 있다고 적어 두는</b> 꼴이 된다.
     */
    /*
     ...
     */
    /*
     * <b>그룹 축이 하나뿐이고 계층이 없다.</b> 원본은 마스터마다 그룹을 <b>여럿</b> 두고
     * (거래처그룹1·2, 품목그룹1·2·3, 프로젝트그룹1·2) 그 위에 <b>계층그룹</b>을 또 얹는다.
     * 우리 그룹 마스터(item_groups·partner_groups)는 <b>평면이고 축이 하나</b>다 —
     * 그래서 그 이름들로 거를 자리가 아예 없다.
     *
     * <p>화면 하나의 일이 아니라 <b>마스터 구조</b>의 일이라 이름으로 한 번에 적는다.
     * 창고등록·거래처관리대장에 진작 같은 이유가 화면별로 적혀 있었는데,
     * 조건 대조표를 24화면 넓히면서 같은 것이 87곳으로 늘었다.
     */
    /*
     * <b>[거래처그룹1]은 뺐다 — 열한 화면 중 열을 만들었다.</b>
     *
     * <p>예전 이유는 '평면이고 <b>번호가 없다</b>' 였다. 뒷말이 틀렸다 — 거래처등록이
     * 원본 이름 그대로 [거래처그룹1] 이라 적고 "우리는 그룹이 하나라 '2' 에 해당하는 칸이
     * 없다" 고 주석까지 달아 두었다. 하나뿐인 그룹에 원본의 '1' 을 붙이는 것이 이 저장소의
     * 방식이다(품목그룹1 과 같다). 없는 것은 2·3 이다.
     *
     * <p>못 만드는 <b>한 화면</b>만 아래 NO_FIELD_ON 에 남겼다 — 뭉뚱그린 '그룹이 하나뿐'
     * 대신 그 화면의 진짜 이유(재고조정 전표에 거래처가 없다)를 적는다.
     */
    ['거래처그룹2', '그룹 축이 <b>하나뿐</b>이다 — 원본의 2·3 에 해당하는 칸이 우리에겐 없다(1 은 있다)'],
    ['거래처계층그룹', '그룹 위에 그룹을 두지 않는다 — 평면이다'],
    /*
     * <b>[품목그룹1]은 뺐다 — 여덟 화면 전부 만들었다.</b>
     *
     * <p>예전 이유는 '그룹 축이 하나뿐이다 — 우리 품목그룹은 평면이고 <b>번호가 없다</b>' 였다.
     * 뒷말이 틀렸다. 품목등록은 원본 이름 그대로 <b>[품목그룹1명]</b> 열을 쓰고 거래처등록도
     * <b>[거래처그룹1]</b> 이라 적는다 — 하나뿐인 그룹에 원본의 '1' 을 붙이는 것이 이 저장소의
     * 방식이다. 없는 것은 <b>2·3</b> 이지 <b>1</b> 이 아니다.
     *
     * <p>품목그룹은 품목 마스터에 붙는 값이라 전표 응답에는 없다. 관리항목과 같은 길로
     * 이었다 — 품목 마스터를 받아 줄의 itemId 로 화면에서 잇는다(utils/itemMgmtItems).
     */
    ['품목그룹2', '그룹 축이 <b>하나뿐</b>이다 — 원본의 2·3 에 해당하는 칸이 우리에겐 없다(1 은 있다)'],
    ['품목그룹3', '위와 같음'],
    ['품목계층그룹', '그룹 위에 그룹을 두지 않는다 — 평면이다'],
    ['프로젝트그룹1', '프로젝트를 그룹으로 묶는 마스터가 없다'],
    ['프로젝트그룹2', '위와 같음'],
    ['창고계층그룹', '창고를 계층으로 묶는 마스터가 없다 — 평면이다'],
    /*
     * <b>품목등록 한 화면에 마흔다섯이 몰려 있다.</b> 원본의 품목 마스터는 단가 열 벌·
     * 추가항목 열여섯 칸·표준원가 넷을 <b>칸 이름째</b> 품목 행에 박아 둔다. 우리는 그 값들을
     * 다른 데 두거나(원가·특별단가) 이름을 짓게 하거나(추가항목) 아예 없다(포털).
     * 화면 사정이 아니라 <b>마스터 설계</b>의 일이라 이름으로 한 번에 적는다.
     *
     * <p><b>추가항목</b> — 우리에게도 있다. 다만 <b>칸 이름을 박지 않는다</b>:
     * Self-Customizing &gt; 사용자정의필드에서 [ITEM]·[PARTNER]·[EMPLOYEE] 에 이름을 지어
     * 정의하면 그 마스터 <b>수정</b> 화면에 그대로 뜬다(#277 에 셋을 넓혔다). 그래서
     * '문자형추가항목3' 같은 <b>원본 이름</b>으로 걸릴 자리가 없는 것이다.
     */
    ['문자형추가항목1', '추가항목은 칸 이름을 박지 않고 사용자정의필드에서 이름을 지어 정의한다'],
    ['문자형추가항목2', '위와 같음'],
    ['문자형추가항목3', '위와 같음'],
    ['문자형추가항목4', '위와 같음'],
    ['문자형추가항목5', '위와 같음'],
    ...
    ['추가항목6', '위와 같음'],
    /*
     * <b>단가 열 벌.</b> 원본은 품목마다 [단가A]~[단가J] 를 두고 거래처에 등급을 붙여 고른다.
     * 우리 품목의 단가는 <b>판매(unitPrice)·구매(purchasePrice) 둘</b>이고, 거래처마다 다른 값은
     * <b>특별단가</b>에서 거래처×품목으로 정한다 — 등급이라는 중간 단계가 없다.
     */
    ['단가A', '품목 단가는 판매·구매 둘이다 — 거래처별 값은 특별단가에서 정한다'],
    ['단가B', '위와 같음'],
    ['단가C', '위와 같음'],
    ['단가D', '위와 같음'],
    ['단가E', '위와 같음'],
    ...
    ['단가J', '위와 같음'],
    /*
     * <b>안전재고를 전표 종류마다 켜고 끈다.</b> 원본은 주문서·판매·생산불출·… 일곱 가지에
     * 각각 '이때 안전재고를 볼까' 체크를 둔다. 우리 품목의 안전재고는 <b>숫자 하나</b>이고,
     * 어느 전표에서 볼지는 고르게 하지 않는다.
     */
    ['안전재고관리-주문서', '안전재고는 숫자 하나다 — 전표 종류별로 켜고 끄지 않는다'],
    ['안전재고관리-판매', '위와 같음'],
    ['안전재고관리-생산불출', '위와 같음'],
    ['안전재고관리-생산입고', '위와 같음'],
    ['안전재고관리-창고이동', '위와 같음'],
    ...
    ['안전재고관리-불량처리', '위와 같음'],
    /*
     * <b>표준원가는 품목이 아니라 기간에 붙는다.</b> 원본은 품목 행에 재료비·노무비·경비
     * 표준원가를 직접 적는다. 우리는 <code>ItemCost</code> 에 <b>기간별로</b> 재료비·노무비·경비를
     * 두고 실제와 나란히 본다(원가관리 화면) — 품목등록에서 정하는 값이 아니다.
     */
    ['재료비표준원가', '표준원가는 품목이 아니라 기간별 원가(ItemCost)에 둔다'],
    ['노무비표준원가', '위와 같음'],
    ['경비표준원가', '위와 같음'],
    /*
     * 아래 셋은 <b>어디에도 없다.</b> 외주비는 원가 요소로 쌓지 않고(재료비·노무비·경비 셋),
     * 가중치는 경비·노무비를 <b>배부</b>할 때 쓰는 값인데 우리 원가는 배부를 하지 않는다.
     */
    ['외주비표준원가', '외주비·배부 가중치를 원가 요소로 두지 않는다'],
    ['경비가중치', '위와 같음'],
    ['표준노무시간(노무비가중치)', '위와 같음'],
    /*
     * <b>C-Portal</b> 은 고객이 직접 들어와 주문을 넣는 원본의 웹 포털이다. 그 최소주문수량은
     * 포털이 주문을 받을 때 보는 값이라, 포털이 없으면 <b>볼 사람이 없다.</b>
     */
    ['C-Portal최소주문수량체크', '고객이 직접 주문을 넣는 웹 포털이 없다'],
    ['C-Portal최소주문수량', '위와 같음'],
    /*
     * 카드사·결제대행사에도 그룹 번호가 붙는다. <b>거래처그룹1·2 와 같은 이야기</b>다 —
     * 우리 카드사·결제대행사는 목록일 뿐 묶는 축이 아예 없다.
     */
    ['카드사그룹1', '카드사·결제대행사를 그룹으로 묶지 않는다'],
    ['카드사그룹2', '위와 같음'],
    ['결제대행사그룹1', '위와 같음'],
    ['결제대행사그룹2', '위와 같음'],
    /*
     * <b>[최초작성자]는 이 목록에서 뺐다.</b> 이름 하나로 13화면을 덮고 있었는데, 그 안에
     * <b>이미 만든 것과 만들 수 있는 것이 섞여</b> 있었다 — 재고이동 현황 다섯·업무일지는
     * 진작 되어 있었고, 작업지시서효율현황·ECDrive·단가요청현황·발주계획현황은 응답이
     * 그 값을 싣고 있는데 화면만 안 쓰고 있었다. 못 만드는 셋만 화면별로 옮겨 적었다
     * (아래 NO_FIELD_ON). 묶음 이유 하나는 그 안에 거짓이 섞여도 아무도 안 본다.
     *
     * <p>[최종수정자]는 남긴다 — 이쪽은 <b>updatedBy 가 저장소 전체에 0건</b>이라
     * 13화면이 다 같은 이유로 못 만든다. BaseTimeEntity 도 updatedAt 만 든다.
     */
    ['최종수정자', '고친 사람을 아무 데도 남기지 않는다 — updatedBy 필드가 없고 BaseTimeEntity 도 시각만 든다'],
  ])
  collectReasons(NO_FIELD)
  /** 화면별로 안 만든 것 — 화면 사정이 있는 것만 여기에. */
  const NO_FIELD_ON = new Map([
    /*
     * <b>판매조회 여섯 — 사본을 실측으로 갈아 끼우며 드러난 것들.</b>
     *
     * <p>2026-09-07 에 원본(E040206) 조건 판의 <b>접힌 줄까지 펼쳐</b> 재 보니 조건이
     * <b>스물넷</b>이었다. 사본에는 열하나만 적혀 있었다 — 사본을 뜰 때 접힌 줄을 안 펼친
     ...
     * 셋 있다: [세금계산서구분](판매는 [회계반영여부]) · [채무번호](판매는 [채권번호]) ·
     * [최종작업일자](판매는 [최종수정일시]). 같은 자리라도 원본이 부르는 이름을 그대로 쓴다.
     */
    ['구매조회|발송여부', '전표를 거래처로 내보내는 기능이 없다 — 보낸 기록 자체가 없다'],
    /*
     * <b>출하조회</b> — 사본에 열뿐이던 것을 실측 스물넷으로 갈아 끼우며 아홉이 남았고,
     * 그 아홉 중 <b>다섯이 이번에 사라졌다</b>. 남겨 둔 이유가 사실이 아니었기 때문이다.
     *
     * <p>[연락처]·[주소]에 '[보내기]에 딸린 칸이라 전표를 내보내는 기능이 없다' 고
     * 적어 두었는데, 둘 다 <b>배송지</b>다 — 출하지시서입력이 받아서 저장하고
     * ShipmentResponse 가 <code>contact</code>·<code>address</code> 로 싣고 있었다.
     * 화면이 받아 두지 않아 못 걸렀을 뿐이다. [거래처관리담당자]에 적힌
     * '거래처 마스터를 따로 받아 이어야 한다' 는 <b>이유가 아니라 할 일</b>이었다 —
     * 관리항목·품목그룹1을 이미 그 길로 이어 놓고 있었다(utils/partnerManagers 로 나눈다).
     * [최초작성일자]·[최종수정일시]는 '응답이 안 싣는다' 가 맞았으므로 <b>실었다</b>.
     *
     * <p>남은 넷은 진짜다. 문자형식·장문형식은 원본의 <b>사용자정의 칸</b>이고
    ...
    /*
     * <b>판매일괄회계반영(E040215) [작성자]</b> — 2026-09-09 실측.
     * 같은 자리를 <b>회계미반영현황(판매)</b>는 [최초작성자] 라 부른다. 한 파일
     * (AccountingReflectionPage)이 원본 넷을 겸하고 라우트도 하나라 이름을 갈라 걸 수 없다 —
     * 조건이 훨씬 많은 회계미반영현황 쪽 이름을 쓴다(차례 예외를 적어 둔 것과 같은 까닭).
     * <b>거르는 일은 이미 된다</b>. 이름만 다르다.
     */
    ['판매일괄회계반영|작성자', '같은 자리를 회계미반영현황(판매)는 [최초작성자] 라 부른다 - 한 파일이 둘을 겸해 둘 다 맞출 수 없다'],
     ...
     * [규격]·[품목구분]·[적요]는 응답 record 에 "원본 조건" 이라고 <b>주석까지 달아 두고</b>
    ...
    ['작업내역조회|최초작성자', '만든 사람을 안 남긴다 — WorkResult 에 createdBy 칸이 없다(생산불출과 같다)'],
    ...
    ['작업내역현황|최초작성자', '만든 사람을 안 남긴다 — <code>WorkResult</code> 에 createdBy 칸이 없다(생산불출과 같다)'],
    ...
    ['생산입고조회|생산입고구분', '원본은 생산입고를 I·II·III 세 화면으로 나눠 두고 그 갈래를 고르게 한다 — 우리는 한 화면이라 나눌 갈래가 없다'],
    ...
    ['생산불출조회|최초작성자', '만든 사람을 안 남긴다 — MaterialIssue 에 createdBy 칸이 없다(판매·구매·출하와 다르다)'],
    ...
    ['불량률파악보고서|정렬/소계기준', '이 표는 <b>불량률 높은 순</b> 하나로 세운다 — 고를 축이 없다'],
    ...
    ['출하지시서조회|오더관리번호', '위와 같음 — 출하 전표에 오더 참조가 없다'],
    ['출하지시서조회|최종수정자', '위와 같음 — 누가 고쳤는지는 안 남긴다'],
    ...
    ['구매현황|진행상태', '구매현황은 <b>줄 단위</b>로 펴 보는 화면이라 전표의 확인 상태를 줄에 붙이지 않는다 — 그 상태로 거르는 자리는 구매조회에 있다'],
    ...
    ['판매조회|삭제구분', '지운 전표를 남기지 않는다 — 지우면 사라진다(소프트 삭제가 아니다)'],
    /*
     * <b>[거래처그룹1] 을 못 만드는 한 화면.</b> 나머지 열은 만들었다(위 전역에서 뺀 까닭).
     * 자가사용현황은 재고조정 전표를 그리는데 <b>StockAdjustment 에 거래처가 없다</b> —
     * 사내에서 쓴 것이라 상대가 없다. 그룹 이전에 거래처 자체를 못 붙인다.
     */
    ['자가사용현황|거래처그룹1', '재고조정 전표에 거래처가 없다 — 사내에서 쓴 것이라 상대가 없다'],
    /*
     * <b>[출력양식]·[생산공정] — 전역에서 화면별로 옮겼다.</b>
     *
     * <p>전역 예외는 <b>한 번 적으면 다시 재지 않는다</b>(위 NO_FIELD 는 낡은 예외 검사를
     * 타지 않는다). 그래서 그 사이 만들어 놓고도 '없다' 로 적혀 있는 것이 생긴다 —
     * 실제로 [생산공정]은 창고등록·BOR 에, [출력양식]은 기안서작성에 <b>이미 있었다</b>.
     * 화면별로 옮기면 그때부터 검사가 다시 잰다.
     *
     * <p>이름이 같아도 화면마다 사정이 다르다는 것도 이렇게 해야 드러난다.
     */
    ['기안서통합관리|출력양식', '인쇄 양식을 고르게 하지 않는다 — 화면마다 양식이 하나다(기안서작성의 [구분]과 다른 칸이다)'],
    ['실제원가현황|생산공정', '우리 원가(표준·실제·차이)에 공정별 값이 없다 — 공정 단위로 쌓지 않는다'],
    ['표준원가현황|생산공정', '위와 같음'],
    ['차이분석|생산공정', '위와 같음'],
    ['품목등록|생산공정', '품목 마스터에 공정을 매어 두지 않는다 — 공정은 작업지시가 든다'],
    /*
     * <b>[최초작성자] 를 못 만드는 셋.</b> 나머지 열은 만들었다(전역 예외에서 뺀 까닭).
     ...
     */
    /*
     * 예전 이유: "우리 근태 자료는 휴가 신청뿐이라 프로젝트에 붙을 일이 없다".
     * <b>전제가 사실이 아니게 됐다.</b> 출퇴근 기록(attendances)이 따로 생겨 78줄이 들어 있다.
     * 결론은 그대로다 — 붙일 칸이 없다. 근거를 지금 사실로 고쳐 적는다.
     */
    ['근태조회|프로젝트', '출퇴근·휴가 어느 쪽에도 프로젝트를 적는 칸이 없다'],
    ['근태현황|프로젝트', '위와 같음'],
    /*
     ...
     */
    ['품질검사현황|구분', '이 화면은 검사 <b>한 건이 한 줄</b>인 목록이라 내역·집계·라인별로 가를 것이 없다 — 품목별로 모아 보는 자리는 불량률파악보고서다'],
    ['품질검사현황|오더관리번호', '검사를 전표에 이어 두지 않아 근거가 될 오더가 없다'],
    ['품질검사현황|거래처관리담당자', '검사에 거래처가 없다 — 우리 검사는 <b>품목</b>에 대고 하지 상대에 대고 하지 않는다'],
    ['품질검사현황|진행상태', '검사는 하면 곧 판정이 난다 — 진행중·완료로 흐르는 단계가 없다'],
    ...
    ['품질검사현황|정렬/소계기준', '이 표는 검사일 내림차순 하나로 세운다 — 고를 축이 없다'],
    /*
     ...
     *
     * <p>우리 <b>[수리일자]</b>(실제로 고친 날)는 원본 A/S<b>수리</b>현황(E040611)의
     * [기준일자] 자리에서 가져온 우리 축이다 — A/S접수현황에는 없다. 그래서
     * 대조표에는 안 적고 화면에는 남겨 둔다(그걸 지우면 이번 달에 고친 건을 못 센다).
     */
    ['A/S접수현황|구분', '이 화면은 접수 <b>한 건이 한 줄</b>인 목록이라 내역·집계·라인별로 가를 것이 없다'],
     ...
     *
     * <p>기간 조건의 <b>이름이 두 화면에서 다르다</b> — 조회는 [기준일자], 현황은 [일자].
     * 창고이동조회/현황에서 겪은 것과 같은 자리다. 한 파일이라 하나만 적을 수 있어
     * 조건이 더 많은 조회 쪽 이름을 그대로 둔다.
     */
    ['매출계획현황|일자', '원본 두 화면이 같은 기간 조건을 다르게 부른다 — 매출계획<b>조회</b>는 [기준일자], 현황은 [일자] 다. 한 파일이 넷을 겸해 이름을 하나만 적을 수 있어 조회 쪽을 따랐다(창고이동현황과 같은 까닭)'],
     ...
     * 사본만 보고 적은 것이라 틀렸다.)
     */
    ['의료기기공급내역보고|납품일자', '공급내역에 날짜가 하나뿐이다 — 전표를 끊은 날이 곧 [기준일자] 다'],
    ...
    ['A/S접수조회|발송여부', '위와 같음'],
    /*
     * <b>[대표품목으로 합산] 셋은 이제 만들 수 있다.</b> "품목 마스터에 대표품목 칸이 없어
     * 묶을 축이 없다" 고 적어 두었는데, #282 에서 품목에 <b>대표품목</b>을 만들면서 그 이유가
     * 사실이 아니게 됐다 — 이유의 근거 검사가 <code>private Item parent</code> 를 짚어 줬다.
     * 예외에서 빼고 <b>아직 못 만든 목록</b>으로 옮긴다. 이유를 고쳐 쓸 것이 아니라 만들 일이다.
     */
    /*
     ...
     * 원본 프로젝트계획 리스트(E040636)를 직접 재어 네 갈래를 V212 로 만들면서 그 이유가
     * 사실이 아니게 됐다 — 이유의 근거 검사가 <code>planPurchase</code> 를 짚어 줬다.
     * 예외에서 빼고 <b>아직 못 만든 목록</b>으로 옮긴다. 이유를 고쳐 쓸 것이 아니라 만들 일이다.
     */
    /*
     * <b>Invoice / Packing List(C000652)</b> — 2026-09-02 원본 실측으로 조건표를 새로 넣으며 드러난 둘.
     */
    ['Invoice / Packing List|Invoice 일자', '원본은 <b>전표일자</b>로 걸고 [Invoice 일자]를 따로 두는데, 우리 수출은 전표일자가 없고 invoiceDate 하나로 돈다 — 그 자리가 곧 [기준일자] 라, 따로 두면 <b>같은 값을 두 번 묻는</b> 꼴이 된다'],
    ['Invoice / Packing List|발송여부', '보낸 기록 자체가 없다 — 우리 수출에는 Email·보내기가 없어 발송/미발송을 가를 축이 없다'],
    /*
     * <b>재고 출력물의 [기타] 안</b> — 2026-09-02 에 원본을 열어 재어 대조표에 풀어 적으면서
     * 드러난 것들. 다섯 화면이 같은 묶음을 두는데 우리가 못 만드는 것은 셋뿐이다.
     */
    ['재고수불부|생산불출/창고이동포함', '우리 재고거래는 생산불출·창고이동을 <b>따로 표시하지 않는다</b> — 그 둘만 가려 넣고 뺄 축이 없다'],
    ...
    ['판매현황|정렬기준', '위와 같음'],
    ['출하현황|시리얼/로트No.',
      '출하 라인에 로트를 달지 않는다 — <code>ShipmentLine</code> 은 품목·수주라인·수량·단가·금액·적요다.'
     ...
     *
     * <p><b>화면코드는 못 읽었다.</b> 이 화면은 메뉴를 눌러도 주소창의 prgId 가 안 바뀌어
     * 앞 화면의 코드(E040419)가 그대로 남는다. 지어내지 않고 비워 둔다.
     */
    ['외주비할인현황|양식', '위와 같음'],
    /*
    ...
    ['생산불출현황|진행상태', '생산 전표에 확인 상태가 없다(판매·구매에만 있다) — 거쳐 갈 단계가 없다'],
    ['생산불출현황|최초작성자', '만든 사람을 안 남긴다 — <code>MaterialIssue</code> 에 createdBy 칸이 없다(판매·구매·출하와 다르다)'],
    ...
    ['생산입고현황|생산입고구분', '원본은 생산입고 I·II·III 중 어느 화면으로 넣었는지를 전표에 남긴다. 우리는 입력 화면이 하나라 그 축이 없다'],
    ['생산입고현황|진행상태', '생산 전표에 확인 상태가 없다(판매·구매에만 있다) — 거쳐 갈 단계가 없다'],
    ...
    ['SW개발일정관리|입력경로', '위와 같음'],
    ['거래처관리대장 II|거래처계층그룹',
      '거래처<b>그룹</b>이 평면이다 — 그룹 위에 그룹을 두지 않는다'
      + '(대표거래처는 거래처끼리의 관계라 다른 것이다. 그건 만들었다)'],
    /*
     * 월별채권/채무증감내역의 표는 <b>달마다 한 줄</b>이다(월 · 전월이월 · 증가 · 감소 · 당월잔액).
     * 월이 곧 축이라 따로 묶을 것이 없다 — 거래처별로 보려면 거래처관리대장으로 간다.
     */
    /*
     * 기타이동(StockAdjustment)에는 <b>상태 칸이 없다</b> — 넣는 순간 재고가 움직이고 끝이라
     * 거쳐 갈 단계가 없다. [진행상태변경] 단추에 진작 같은 이유를 적어 두었다.
     */
    ['자가사용현황|진행상태', '기타이동에는 상태 칸이 없다 — 넣는 순간 재고가 움직이고 끝이다'],
    ['불량처리현황|진행상태', '위와 같음'],
    ['대체사용현황|진행상태', '위와 같음'],
    ['폐기현황|진행상태', '위와 같음'],
    ['월별채권증감내역|정렬/소계기준', '표가 달마다 한 줄이라 월이 곧 축이다 — 따로 묶을 것이 없다'],
    ['월별채무증감내역|정렬/소계기준', '위와 같음'],
    /*
     * <b>기타이동에는 거래처가 없다.</b> 자가사용·불량처리·폐기는 <b>우리 안에서</b> 재고가
     * 움직이는 일이라 상대가 없다 — StockAdjustment 에 거래처 칸 자체가 없다.
     */
    ['자가사용현황|거래처', '기타이동에는 거래처가 없다 — 우리 안에서 재고가 움직이는 일이다'],
    /*
     * <b>[거래처관리담당자]도 같은 까닭이다.</b> 그 담당자는 <b>거래처 마스터</b>에 붙어
     * 있는데 기타이동 전표에는 거래처가 없다 — 이을 상대가 없으니 좁힐 것도 없다.
     */
    ['자가사용현황|거래처관리담당자', '전표에 거래처가 없어 그 거래처의 담당자도 없다'],
    ['불량처리현황|거래처', '위와 같음'], ['대체사용현황|거래처', '위와 같음'],
    ['폐기현황|거래처', '위와 같음'], ['재고조정현황|거래처', '위와 같음'],
    /*
     * <b>근태에 내/외근도 업무일/휴일도 없다.</b> 우리 Attendance 는 출근·퇴근 시각과
     * 상태(정상·지각·결근)뿐이다 — 그 둘로 가를 값이 아예 없다.
     */
    ['출/퇴근현황(ID)|내/외근구분', '근태에 내/외근 칸이 없다 — 출퇴근 시각과 상태뿐이다'],
    ['출/퇴근현황(ID)|업무일/휴일구분', '근태에 업무일/휴일 칸이 없다 — 달력을 따로 두지 않는다'],
    ['지각현황(ID)|내/외근구분', '위와 같음'], ['지각현황(ID)|업무일/휴일구분', '위와 같음'],
    ['출퇴근/근태/일정현황(ID)|내/외근구분', '위와 같음'],
    /*
     * <b>월별채권/채무증감내역은 한 해를 열두 달로 편다.</b> 기간이 아니라 [연도]로 고른다 —
     * 날짜 구간을 주면 그 표의 열두 줄이 무엇을 뜻하는지 알 수 없어진다.
     */
    ['월별채권증감내역|기준일자', '한 해를 열두 달로 펴는 표라 [연도]로 고른다'],
    ['월별채무증감내역|기준일자', '위와 같음'],
    /*
     * <b>근태 마스터가 없다.</b> 원본은 근태를 <b>항목</b>으로 등록해 쓴다 —
     * [근태항목]을 만들고 그것을 [근태그룹]으로 묶고, 전표에 [근태구분]·[근무구분]으로
     * 고른다. 우리 <code>Attendance</code> 는 출근·퇴근 시각 둘뿐이고 상태(정상·지각·조퇴·
     * 결근)는 그 시각에서 <b>계산</b>한다 — 고를 항목이 아예 없다.
     *
     * <p>화면의 [상태]가 그 계산 결과다. 이름이 비슷해 [근태구분]에 갖다 붙일 뻔했는데,
     * 원본은 [상태]와 [근태구분]을 <b>나란히 둘 다</b> 둔다 — 다른 것이다.
     */
    ['출퇴근/근태/일정현황(ID)|근태항목', '근태를 항목으로 등록하지 않는다 — 출퇴근 시각에서 상태를 계산한다'],
    ['출퇴근/근태/일정현황(ID)|근태그룹', '위와 같음 — 묶을 항목이 없다'],
    ['출퇴근/근태/일정현황(ID)|근태구분', '위와 같음. 화면의 [상태]는 계산 결과이지 고르는 구분이 아니다'],
    ['출퇴근/근태/일정현황(ID)|근무구분', '위와 같음 — 정상근무·특근을 가르는 값이 없다'],
    /*
     * <b>[휴가항목]</b> — 휴가는 <code>VacationRequest</code> 에 따로 있고(연차·반차·병가),
     * 이 화면은 <b>근태와 일정</b>을 합친 것이다. 휴가 줄이 아예 안 섞이므로 거를 것이 없다.
     */
    ['출퇴근/근태/일정현황(ID)|휴가항목', '이 화면은 근태와 일정만 합친다 — 휴가는 휴가신청에 따로 있다'],
    ['휴가잔여일수현황|상태', '잔여일수 응답은 휴가명·사원·부서·일수를 준다 — 줄에 상태가 없다'],
    /*
     * 원본 <b>거래처관리대장 II</b> 의 [기준일자]는 <b>그날짜 기준 잔액</b>을 보려는 것이다.
     * 우리 거래처 목록에는 잔액을 싣지 않는다 — 채권·채무는 거래처별채권/채무에서 보고,
     * 그 화면에는 [기준일자]가 있다. 자를 값이 없는 자리에 날짜만 두면 아무 일도 안 한다.
     */
    ['거래처관리대장 II|기준일자', '거래처 목록에 잔액을 싣지 않는다 — 잔액은 거래처별채권/채무에서 본다'],
    /*
     * <b>매출계획비교표</b> 는 계획을 <b>연·월</b> 단위로 세운다(planYear·planMonth).
     * 날짜 구간으로 자를 것이 없어 [계획연도]로 고른다.
     */
    ['매출계획비교표|기준일자', '계획이 연·월 단위라 [계획연도]로 고른다 — 날짜 구간으로 자를 것이 없다'],
    ...
    /*
     * <b>[오더관리번호] — 발주·기타이동 전표에는 오더 번호 칸이 없다.</b>
     * 오더관리는 <b>수주</b>에 붙는다(SalesOrder 가 orderTypeId·stage 를 든다).
     * 발주서·재고조정에는 그 칸이 아예 없어서 오더로 좁힐 것이 없다.
     * 작업지시에 진작 같은 이유를 적어 두었다.
     *
     * <p>처음에는 "전표에 오더 번호를 붙이지 않는다" 고 <b>너무 넓게</b> 적었는데,
     * 이유의 근거 검사가 <code>orderTypeId</code> 가 있다고 짚어 줬다 — 수주 것이다.
     */
    ['작업지시서효율현황|오더관리번호', '작업지시를 수주(오더)와 잇지 않는다'],
    ['단가요청현황|오더관리번호', '발주 전표에는 오더 번호 칸이 없다 — 오더관리는 수주에 붙는다'],
    ['발주계획현황|오더관리번호', '위와 같음'],
    /*
     * <b>[참조] — 발주 전표에 그런 칸이 없다.</b> 원본은 전표마다 다른 문서를 가리키는
     * [참조] 를 두는데, 우리 <code>PurchaseOrder</code> 에는 적요(remark) 말고 문서를
     * 가리키는 칸이 없다. 만들어 온 전표를 잇는 <code>convertedPurchaseId</code> 는
     * <b>구매전표로 바뀐 뒤</b>에 생기는 것이라 발주계획·단가요청 단계에는 늘 비어 있다.
     */
 ...
 */
{
  ...
  const ORDER_SKIP = new Map([
    /*
     * <b>등록 폼의 칸 차례는 원본과 다르게 묶었다.</b> 사본의 이 둘은 조회 조건이 아니라
     * <b>등록 폼</b>의 칸이다 — 우리는 뜻이 붙는 것끼리 모았다(주소1 옆에 그 우편번호,
     * 재고수량관리 옆에 사용구분). 원본 차례대로 늘어놓으면 주소와 우편번호가 갈라진다.
     * 조건 대조표를 24화면 넓히며 걸린 것이다.
     */
    /*
     * <b>한 파일이 겸하는 다섯 화면이 서로 다른 차례를 쓴다</b>(사본 실측) —
     * 자가사용은 <b>규격</b>·담당자·적요, 불량처리는 담당자·적요·<b>규격</b>,
     * 대체사용·폐기는 담당자·<b>규격</b>·적요다. 하나만 고를 수 있어 둘이 겹치는
     * 대체사용·폐기에 맞췄다 — 남는 둘은 <b>원본끼리</b> 어긋난 것이라 우리가 맞출 수 없다.
     */
    /*
     * 단가요청현황·발주계획현황도 <b>프로젝트를 다른 자리에 둔다</b>(사본 실측) —
     * 발주계획은 창고·<b>프로젝트</b>·거래처·품목, 단가요청은 거래처·품목·<b>프로젝트</b> 다.
     * 조건이 더 많은 발주계획현황에 맞췄다.
     */
    ['단가요청현황', '같은 파일의 발주계획현황과 프로젝트 자리가 다르다 — 조건이 더 많은 쪽에 맞췄다'],
    ['자가사용현황', '원본 다섯이 서로 다른 차례다 — 둘이 겹치는 대체사용·폐기에 맞췄다'],
    ['불량처리현황', '위와 같음'],
    ['거래처등록', '조회 조건이 아니라 <b>등록 폼</b>의 칸 차례다 — 주소와 우편번호처럼 뜻이 붙는 것끼리 모았다'],
    ['품목등록', '위와 같음'],
    ['매출계획비교표',
 ...
 */
{
  ...
  const TITLE_SKIP = new Map([
    ...
    /* 지도를 넓히며 걸린 것 — 한 파일이 견적서·견적서조회·견적서현황 셋을 겸한다. */
    ['견적서현황', '견적서 한 화면이 입력·조회·현황을 겸한다 — 제목은 그 주인인 [견적서]다'],
    ...
    ['프로젝트계획', '우리 제목은 [프로젝트계획 리스트] 다 — 원본 화면 제목이 그것이다(2026-09-01 E040636 실측). 메뉴 이름만 [프로젝트계획] 이다'],
 ...
 */
{
  ...
  const NO_OPTION = new Map([
    /*
     * <b>이유가 두 번 썩었다.</b> 처음엔 '거래처에 계층이 없다' 고 적어 뒀는데 대표거래처는
     * 그 사이 생겼고(BusinessPartner.parent · 거래처리스트 [관계설정]), 그래서
     * "[대표거래처로 합산] <b>체크</b> 하나가 곧 이 선택이라 두 번 그리지 않는다" 로 고쳐 적었다.
     * 그것도 틀렸다 — 2026-09-02 에 원본 채권현황(E040721)을 열어 보니 <b>체크박스가 없다.</b>
     * 그 줄 자체가 라디오 두 알이고 [거래처관계기준]이 눌린 채로 열린다. 우리는 원본에 없는
     * 체크박스를 만든 뒤, 원본에 있는 라디오를 "중복" 이라며 예외로 덮어 둔 것이다.
     * 그 바람에 <b>[개별거래처기준]이라는 말이 화면에 한 번도 안 나왔고</b> 기본값도 뒤집혀 있었다.
     *
     * <p>채권현황·채무현황(ArApStatusPage)은 이번에 라디오로 고쳤다 — 예외에서 뺀다.
     * 아래 셋은 <b>아직 원본을 안 열어 봤다.</b> 같은 줄일 것이 거의 확실하지만 재지 않은 것을
     * 고치지는 않는다. 열어 보는 대로 하나씩 뺀다.
     * 거래처관리대장 II 는 거래처등록 화면이라 합칠 잔액 자체가 없다.
     */
    ...
    ['구매단가일괄변경|환율변경', '매입 전표에 통화 칸이 없어 바꿀 환율이 없다(수출오더에는 있다)'],
    ['판매단가일괄변경|단가변경', '위와 같음'], ['판매단가일괄변경|환율변경', '위와 같음'],
    ['미삭제', '지운 전표를 남겨 두지 않는다 — 지우면 없어진다'],
    ['삭제', '위와 같음'],
  ])
  collectReasons(NO_OPTION)
...
// ── 1-s4) 빈 칸에 줄표를 그리지 않나 ────────────────────────────────────
console.log('\n■ 빈 칸에 줄표를 그리지 않나')

/*
 * 원본은 값이 없는 칸을 <b>그냥 비워 둔다.</b> 사본 26,209개 칸 가운데 줄표 하나만
 * 들어 있는 칸은 <b>0개</b>다(빈 칸 7,044 · &nbsp; 5,319). N/A 도 없다.
 *
 * <p>우리는 '-' 와 '—' 를 없음 표시로 144자리에 쓰고 있었다. 같은 뜻이 세 모양이었고
 * (빈칸·'-'·'—'), 줄표가 든 칸은 <b>값이 있는 것처럼</b> 보여서 표를 훑을 때 눈이 걸린다.
 *
 * <p>줄표 자체를 금하는 것이 아니라 <b>없음을 뜻하는 자리</b>(?? · || · 삼항의 else)만 본다 —
 * '2026/06/01 ~ 2026/07/16' 처럼 글자 안에 든 줄표는 그대로 둔다.
 */
{
  const bad = []
  for (const f of walk(join('frontend', 'src', 'pages'))) {
    ...
    lines.forEach((L, i) => {
      if (!/<td/.test(L)) return
      if (!/(\?\?|\|\||:)\s*'(-|—)'/.test(L)) return
      bad.push(`${f.split(/[\\/]/).slice(-2).join('/')}:${i + 1}`)
    })
  }
...
// ── 1-s13) 고른 줄이 있어야 도는 버튼을 미리 잠갔나 ──────────────────────
console.log('\n■ 고른 줄이 있어야 도는 버튼을 미리 잠갔나')

/*
 * 원본은 <b>고른 줄에 대고 하는 버튼</b>([선택삭제]·[진행상태변경]·[라벨변경] 같은 것)을
 * 아무것도 안 골랐을 때 회색으로 잠가 둔다. 사본에서 실제로 그렇게 잡혀 있다 —
 * 선택삭제 100개 중 7개, 진행상태변경 3개, 다른전표생성 3개가 안 눌리는 상태였다.
 *
 * <p>우리는 열 자리 모두 <b>누를 수 있게</b> 두고, 누른 다음에야 "고르세요" 라고 되물었다.
 * 그건 사람에게 헛걸음을 시키는 것이다 — 누르기 전에 못 누른다고 보여 주는 편이 낫다.
 *
 * <p>버튼 이름이 <b>선택…</b> 으로 시작하거나, 누르는 함수 이름이 Selected·Checked 로
 * 끝나는 것을 고른 줄에 대고 하는 버튼으로 본다. 그 버튼에는 disabled 가 붙어 있어야 한다.
 */
{
  const bad = []
  let 잰버튼 = 0
  for (const f of walk(join('frontend', 'src', 'pages'))) {
    if (!f.endsWith('.tsx')) continue
    const src = readFileSync(f, 'utf8')
    const 파일 = f.split(/[\\/]/).pop()

    /* 1) EcListShell 의 actions 로 넘긴 것 */
    for (const m of src.matchAll(/\{\s*label:\s*'(선택[^']*|진행상태변경|라벨변경|결재\/검토완료)'[^}]*\}/g)) {
      잰버튼 += 1
      if (!/disabled\s*:/.test(m[0])) bad.push(`${파일}  [${m[1]}] 이 안 잠긴다`)
    }
    /* 2) 화면이 직접 그린 <button> */
    for (const m of src.matchAll(/<button[^>]*onClick=\{\(\)\s*=>\s*(?:void\s+)?\w*(?:Selected|Checked)\(\)[^>]*>([^<]{1,14})<\/button>/g)) {
      잰버튼 += 1
      if (!/\bdisabled=/.test(m[0])) bad.push(`${파일}  [${m[1].trim()}] 이 안 잠긴다`)
    }
  }
...
// ── 1-s15) 이름이 겹치는 화면 ─────────────────────────────────────────────
console.log('\n■ 사본에서 이름이 겹치는 화면을 그대로 쓰고 있지 않나')

/*
 * <b>사본의 h3 이름은 유일하지 않다.</b> [출력물] 덩어리가 둘인데 화면코드가 다르다 —
 * 우리 [출력물](재고 II 장표)은 ESZ001M 이고 조건이 없는데, <b>EPD026R(휴가 출력물)</b> 의
 * 조건(휴가코드·재직구분 …)이 같은 이름으로 담겨 있다.
 *
 * <p>조건 대조표를 24화면 넓힐 때 이름만 보고 뽑아 <b>남의 화면 조건</b>을 우리 출력물 것으로
 * 걸었다. 여덟 줄이 "없는 조건" 으로 세어졌고, 그걸 만들려다 알았다.
 *
 * <p>그래서 겹치는 이름을 ecount-name-collisions.json 에 적어 두고, <b>거기 적힌 이름을
 * 대조표가 쓰고 있으면</b> 왜 괜찮은지 note 로 말하게 한다. 안 적고 쓰면 걸린다.
 */
{
  const col = JSON.parse(readFileSync(join('qa', 'fixtures', 'ecount-name-collisions.json'), 'utf8'))
  const ff2 = JSON.parse(readFileSync(join('qa', 'fixtures', 'ecount-form-fields.json'), 'utf8'))
  const used = Object.keys(col).filter((n) => ff2[n])
  const noNote = used.filter((n) => !(col[n].note ?? '').trim())
  eq('겹치는 이름을 대조표가 쓸 때는 왜 괜찮은지 적었다', noNote.join(', ') || '없음', '없음')
  /* 우리 [출력물]에는 조건이 없다 — 다시 걸리면 남의 화면 것을 또 가져온 것이다. */
  eq('출력물 조건을 다시 걸지 않았다', ff2['출력물'] ? '걸려 있다' : '없음', '없음')
}
 ...
 * 잴 수 있는 것만 적는다.
 *
 * <p><b>증거가 붙어 있다고 그 이유가 참인 것은 아니다.</b> 2026-09-07 에 걸린 것:
 * '발주 전표에 다른 문서를 가리키는 칸이 없다'(단가요청현황·발주계획현황의 [참조])에
 * <code>reference</code>·<code>refNo</code>·<code>refDocument</code> 가 PurchaseOrder 에
 * 없다는 증거를 달아 두었다. 셋 다 정말 없다 — 그런데 <b>그 칸의 이름은
 * <code>remark</code></b> 였다. 발주서입력의 [참조]가 그것이고 응답에도 실려 온다.
 * <b>없는 이름 셋을 골라 대면 어떤 거짓말이든 증명된다.</b> 증거는 '이 이름이 없다' 가
 * 아니라 '이 값을 담을 자리가 없다' 를 보여야 하는데, 이름만으로는 그걸 못 가른다.
 * 그러니 이 검사가 조용하다고 이유가 사실인 것은 아니다 — <b>여전히 사람이 다시 읽어야 한다.</b>
 */
{
   ...
   */
  /*
   * <b>같은 키를 두 번 적으면 앞엣것이 조용히 죽는다.</b> Map 은 뒤엣것만 남긴다 —
   * 그래서 공들여 쓴 이유가 아무도 안 읽는 글이 되고, 뒤에 적힌 엉성한 한 줄이
   * 그 자리를 대신한다. 실제로 [구매단가일괄변경|환율] 이 그랬고, 남은 쪽이
   * 사실도 아니었다. 사람 눈으로는 수백 줄 떨어진 두 줄을 못 맞춘다.
   */
  {
    const src = readFileSync(join('qa', 'ui-check.mjs'), 'utf8')
    const dup = []
    const RE_MAP = /const ([A-Z_]+) = new Map\(\[\r?\n([\s\S]*?)\r?\n {2}\]\)/g
    for (const m of src.matchAll(RE_MAP)) {
      const seen = new Set()
      for (const e of m[2].matchAll(/^\s*\['([^']+)'/gm)) {
        if (seen.has(e[1])) dup.push(`${m[1]}  ['${e[1]}'] — 같은 키가 두 번 적혀 있다(앞엣것이 죽는다)`)
        seen.add(e[1])
      }
    }
