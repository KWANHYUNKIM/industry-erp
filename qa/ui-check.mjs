 ...
 */
{
  ...
  const NO_COLUMN = new Map([
     ...
     */
    ['품질검사요청입력|검사방법', '전수·샘플링을 적는 칸이 없다 — 우리 [검사구분]은 수입·공정·출하로 <b>다른 축</b>이다'],
    ['품질검사요청입력|프로젝트', '검사요청 전표에 프로젝트 칸이 없다'],
    ...
    ['매출계획|일자-No.', '계획은 <b>연월</b>로 잡는다 — 날짜와 번호를 가진 전표가 아니다'],
    ['매출계획조회|일자-No.', '위와 같음'],
    ['매출계획|담당자명', '매출계획에 담당자를 달지 않는다'],
    ['매출계획조회|담당자명', '위와 같음'],
     ...
     */
    ['A/S접수|접수증', '접수증 인쇄물을 만들지 않는다'],
    ['A/S접수|상세내역', '줄을 눌러 펼치는 상세 창이 없다 — [부품] 창이 그 자리를 한다'],
    ['A/S접수|생성한 전표', '그 접수에서 나온 전표를 이어 두지 않는다 — 부품 소모가 재고만 깎는다'],
    ['A/S접수조회|접수증', '위와 같음'],
    ['A/S접수조회|상세내역', '위와 같음'],
    ['A/S접수조회|생성한 전표', '위와 같음'],
    ...
    ['쇼핑몰품목코드연결|쇼핑몰코드', '쇼핑몰을 이름으로 적는다 — 코드를 가진 몰 마스터가 없다'],
    ['시리얼/로트No.등록|상세내역', '로트 한 줄을 눌러 펼치는 상세를 만들지 않았다'],
    ['생산불출|일자-No.', '생산불출 전표에 번호를 매기지 않는다 — 줄 하나가 곧 한 건이라 [일자]만 찍는다(생산불출조회와 같다)'],
    ['단가요청진행단계|수취금액', '단가요청에 받은 금액을 적는 칸이 없다 — 확정 단가만 든다'],
    ['단가요청진행단계|이력', '요청이 어떻게 오갔는지의 <b>이력</b>을 남기지 않는다 — 지금 단계만 든다'],
    /*
 ...
 */
{
  ...
  const NO_FIELD_ON = new Map([
    ...
    ['외화등록|환율', '환율은 <b>날짜별 고시</b>라 통화 마스터의 값이 아니다 — 우리는 [환율등록] 탭에서 따로 든다'],
    /* 개념이 없는 나머지. */
     ...
     */
    ['거래처관리대장 I|대표거래처로 합산', '거래처 관계 마스터(대표거래처)가 없다 — 늘 개별 거래처로 본다'],
    ['거래처관리대장 II|대표거래처로 합산', '위와 같음'],
    ['거래처별채권|대표거래처로 합산', '위와 같음'],
    ['거래처별채무|대표거래처로 합산', '위와 같음'],
